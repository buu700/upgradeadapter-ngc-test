...
(function(process) {
  !function(global, factory) {
    ...
    function wrapIntoObservable(value) {
      return value instanceof rxjs_Observable.Observable ? value : value instanceof Promise ? rxjs_observable_fromPromise.fromPromise(value) : rxjs_observable_of.of(value);
    }
    function createEmptyUrlTree() {
      return new UrlTree(new UrlSegmentGroup([], {}), {}, null);
    }
    function containsTree(container, containee, exact) {
      return exact ? equalSegmentGroups(container.root, containee.root) : containsSegmentGroup(container.root, containee.root);
    }
    ...
    function pairs(obj) {
      var res = [];
      for (var prop in obj)
        obj.hasOwnProperty(prop) && res.push(new Pair(prop, obj[prop]));
      return res;
    }
    ...
    function match(segmentGroup, route, segments) {
      ...
      for (var path = route.path,
        ...
        if (!isPosParam && p !== current.path)
          return noMatch;
        isPosParam && (positionalParamSegments[p.substring(1)] = current), consumedSegments.push(current), currentIndex++;
      }
      return "full" === route.pathMatch && (segmentGroup.hasChildren() || currentIndex < segments.length) ? {
        matched: !1,
        consumedSegments: [],
        lastChild: 0,
        positionalParamSegments: {}
    ...
    function applyRedirectCommands(segments, redirectTo, posParams) {
      ...
      return "" === r ? [] : createSegments(redirectTo, r.split("/"), segments, posParams);
    }
    ...
    function createEmptyStateSnapshot(urlTree, rootComponent) {
      var emptyParams = {},
          ...
          emptyQueryParams = {},
          fragment = "",
          activated = new ActivatedRouteSnapshot([], emptyParams, emptyQueryParams, fragment, emptyData, PRIMARY_OUTLET, rootComponent, null, urlTree.root, (-1), InheritedResolve.empty);
    ...
    function advanceActivatedRoute(route) {
      route.snapshot ? (shallowEqual(route.snapshot.queryParams, route._futureSnapshot.queryParams) || route.queryParams.next(route._futureSnapshot.queryParams), route.snapshot.fragment !== route._futureSnapshot.fragment && route.fragment.next(route._futureSnapshot.fragment), shallowEqual(route.snapshot.params, route._futureSnapshot.params) || (route.params.next(route._futureSnapshot.params), route.data.next(route._futureSnapshot.data)), shallowEqualArrays(route.snapshot.url, route._futureSnapshot.url) || route.url.next(route._futureSnapshot.url), route.snapshot = route._futureSnapshot) : (route.snapshot = route._futureSnapshot, route.data.next(route._futureSnapshot.data));
    }
    function createRouterState(curr, prevState) {
      var root = createNode(curr._root, prevState ? prevState._root : void 0);
    ...
    function createUrlTree(route, urlTree, commands, queryParams, fragment) {
      ...
      if (validateCommands(normalizedCommands), navigateToRoot(normalizedCommands))
        return tree(urlTree.root, new UrlSegmentGroup([], {}), urlTree, queryParams, fragment);
      var startingPosition = findStartingPosition(normalizedCommands, urlTree, route),
          segmentGroup = startingPosition.processChildren ? updateSegmentGroupChildren(startingPosition.segmentGroup, startingPosition.index, normalizedCommands.commands) : updateSegmentGroup(startingPosition.segmentGroup, startingPosition.index, normalizedCommands.commands);
    ...
    function validateCommands(n) {
      ...
      var c = n.commands.filter(function(c) {
        return "object" == typeof c && void 0 !== c.outlets;
      });
      if (c.length > 0 && c[0] !== n.commands[n.commands.length - 1])
        throw new Error("{outlets:{}} has to be the last command");
    }
    function isMatrixParams(command) {
      return "object" == typeof command && void 0 === command.outlets && void 0 === command.segmentPath;
    }
    function tree(oldSegmentGroup, newSegmentGroup, urlTree, queryParams, fragment) {
      return urlTree.root === oldSegmentGroup ? new UrlTree(newSegmentGroup, stringify(queryParams), fragment) : new UrlTree(replaceSegment(urlTree.root, oldSegmentGroup, newSegmentGroup), stringify(queryParams), fragment);
    }
    ...
    function navigateToRoot(normalizedChange) {
      return normalizedChange.isAbsolute && 1 === normalizedChange.commands.length && "/" == normalizedChange.commands[0];
    }
    function normalizeCommands(commands) {
      if ("string" == typeof commands[0] && 1 === commands.length && "/" == commands[0])
        return new NormalizedNavigationCommands((!0), 0, commands);
      for (var numberOfDoubleDots = 0,
          isAbsolute = !1,
          res = [],
          _loop_1 = function(i) {
            var c = commands[i];
            if ("object" == typeof c && void 0 !== c.outlets) {
              var r_1 = {};
              return forEach(c.outlets, function(commands, name) {
                "string" == typeof commands ? r_1[name] = commands.split("/") : r_1[name] = commands;
              }), res.push({outlets: r_1}), "continue";
            }
            if ("object" == typeof c && void 0 !== c.segmentPath)
              return res.push(c.segmentPath), "continue";
            if ("string" != typeof c)
              return res.push(c), "continue";
            if (0 === i)
              for (var parts = c.split("/"),
                  j = 0; j < parts.length; ++j) {
                var cc = parts[j];
                0 == j && "." == cc || (0 == j && "" == cc ? isAbsolute = !0 : ".." == cc ? numberOfDoubleDots++ : "" != cc && res.push(cc));
              }
            else
              res.push(c);
          },
    ...
    function findStartingPosition(normalizedChange, urlTree, route) {
      if (normalizedChange.isAbsolute)
        return new Position(urlTree.root, (!0), 0);
    ...
    function getPath(command) {
      return "object" == typeof command && command.outlets ? command.outlets[PRIMARY_OUTLET] : "" + command;
    }
    ...
    function match$1(segmentGroup, route, segments, parent) {
      ...
      for (var path = route.path,
        ...
        if (!isPosParam && p !== current.path)
          throw new NoMatch$1;
        isPosParam && (posParameters[p.substring(1)] = current.path), consumedSegments.push(current), currentIndex++;
      }
      if ("full" === route.pathMatch && (segmentGroup.hasChildren() || currentIndex < segments.length))
        throw new NoMatch$1;
    ...
    function setupRouter(ref, urlSerializer, outletMap, location, injector, loader, compiler, config, opts) {
      ...
      return opts.errorHandler && (r.errorHandler = opts.errorHandler), opts.enableTracing && r.events.subscribe(function(e) {
        console.group("Router Event: " + e.constructor.name), console.log(e.toString()), console.log(e), console.groupEnd();
      }), r;
    }
    ...
    function initialRouterNavigation(router, ref, preloader, opts) {
      return function() {
        router.resetRootComponentType(ref.componentTypes[0]), preloader.setUpPreloading(), opts.initialNavigation === !1 ? router.setUpLocationChangeListener() : router.initialNavigation();
      };
    }
    ...
    var __extends = this && this.__extends || function(d, b) {
      ...
      d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
        ...
        NavigationCancelingError = function(_super) {
          ...
          return __extends(NavigationCancelingError, _super), NavigationCancelingError.prototype.toString = function() {
            return this.message;
          }, NavigationCancelingError;
        }(Error),
        ROUTES = new _angular_core.OpaqueToken("ROUTES"),
        LoadedRouterConfig = function() {
          function LoadedRouterConfig(routes, injector, factoryResolver) {
            this.routes = routes, this.injector = injector, this.factoryResolver = factoryResolver;
          }
          return LoadedRouterConfig;
        }(),
        RouterConfigLoader = function() {
          function RouterConfigLoader(loader, compiler) {
            this.loader = loader, this.compiler = compiler;
          }
          return RouterConfigLoader.prototype.load = function(parentInjector, loadChildren) {
        ...
        UrlTree = function() {
          ...
          return UrlTree.prototype.toString = function() {
        ...
        UrlSegment = function() {
          ...
          return UrlSegment.prototype.toString = function() {
        ...
        ApplyRedirects = function() {
          function ApplyRedirects(injector, configLoader, urlTree, config) {
            this.injector = injector, this.configLoader = configLoader, this.urlTree = urlTree, this.config = config, this.allowRedirects = !0;
          }
          return ApplyRedirects.prototype.apply = function() {
            var _this = this,
                expanded$ = this.expandSegmentGroup(this.injector, this.config, this.urlTree.root, PRIMARY_OUTLET),
            ...
            return 0 === segmentGroup.segments.length && segmentGroup.hasChildren() ? rxjs_operator_map.map.call(this.expandChildren(injector, routes, segmentGroup), function(children) {
              return new UrlSegmentGroup([], children);
            }) : this.expandSegment(injector, segmentGroup, routes, segmentGroup.segments, outlet, !0);
            ...
            return rxjs_operator_catch._catch.call(first$, function(e, _) {
              throw e instanceof rxjs_util_EmptyError.EmptyError ? new NoMatch(segmentGroup) : e;
            });
            ...
            return "**" === route.path ? this.expandWildCardWithParamsAgainstRouteUsingRedirect(injector, routes, route, outlet) : this.expandRegularSegmentAgainstRouteUsingRedirect(injector, segmentGroup, routes, route, segments, outlet);
            ...
            return this.expandSegment(injector, group, routes, newSegments, outlet, !1);
            ...
            return route.redirectTo.startsWith("/") ? absoluteRedirect(newSegments) : this.expandSegment(injector, segmentGroup, routes, newSegments.concat(segments.slice(lastChild)), outlet, !1);
          }, ApplyRedirects.prototype.matchSegmentAgainstRoute = function(injector, rawSegmentGroup, route, segments) {
            var _this = this;
            if ("**" === route.path)
              return route.loadChildren ? rxjs_operator_map.map.call(this.configLoader.load(injector, route.loadChildren), function(r) {
                return route._loadedConfig = r, rxjs_observable_of.of(new UrlSegmentGroup(segments, {}));
            ...
            var rawSlicedSegments_1 = segments.slice(lastChild),
                childConfig$ = this.getChildConfig(injector, route);
            return rxjs_operator_mergeMap.mergeMap.call(childConfig$, function(routerConfig) {
              var childInjector = routerConfig.injector,
                  ...
                  _a = split(rawSegmentGroup, consumedSegments_1, rawSlicedSegments_1, childConfig),
              ...
              var expanded$ = _this.expandSegment(childInjector, segmentGroup, childConfig, slicedSegments, PRIMARY_OUTLET, !0);
            ...
            var _this = this;
            return route.children ? rxjs_observable_of.of(new LoadedRouterConfig(route.children, injector, null)) : route.loadChildren ? rxjs_operator_mergeMap.mergeMap.call(runGuards(injector, route), function(shouldLoad) {
              return shouldLoad ? route._loadedConfig ? rxjs_observable_of.of(route._loadedConfig) : rxjs_operator_map.map.call(_this.configLoader.load(injector, route.loadChildren), function(r) {
                return route._loadedConfig = r, r;
              }) : canLoadFails(route);
            }) : rxjs_observable_of.of(new LoadedRouterConfig([], injector, null));
          }, ApplyRedirects;
        }(),
        Tree = function() {
          function Tree(root) {
        ...
        RouterState = function(_super) {
          function RouterState(root, snapshot) {
            _super.call(this, root), this.snapshot = snapshot, setRouterStateSnapshot(this, root);
          }
          return __extends$1(RouterState, _super), RouterState.prototype.toString = function() {
        ...
        InheritedResolve = function() {
          function InheritedResolve(parent, current) {
            this.parent = parent, this.current = current, this.resolvedData = {};
          }
          return Object.defineProperty(InheritedResolve.prototype, "flattenedResolvedData", {
            get: function() {
        ...
        RouterStateSnapshot = function(_super) {
          function RouterStateSnapshot(url, root) {
            _super.call(this, root), this.url = url, setRouterStateSnapshot(this, root);
          }
          return __extends$1(RouterStateSnapshot, _super), RouterStateSnapshot.prototype.toString = function() {
            return serializeNode(this._root);
          }, RouterStateSnapshot;
        }(Tree),
        NormalizedNavigationCommands = function() {
          function NormalizedNavigationCommands(isAbsolute, numberOfDoubleDots, commands) {
            this.isAbsolute = isAbsolute, this.numberOfDoubleDots = numberOfDoubleDots, this.commands = commands;
          }
        ...
        InheritedFromParent = function() {
          function InheritedFromParent(parent, snapshot, params, data, resolve) {
            this.parent = parent, this.snapshot = snapshot, this.params = params, this.data = data, this.resolve = resolve;
          }
          return Object.defineProperty(InheritedFromParent.prototype, "allParams", {
            get: function() {
          ...
          return Recognizer.prototype.recognize = function() {
            try {
              var rootSegmentGroup = split$1(this.urlTree.root, [], [], this.config).segmentGroup,
                  children = this.processSegmentGroup(this.config, rootSegmentGroup, InheritedFromParent.empty(null), PRIMARY_OUTLET),
                  root = new ActivatedRouteSnapshot([], Object.freeze({}), Object.freeze(this.urlTree.queryParams), this.urlTree.fragment, {}, PRIMARY_OUTLET, this.rootComponentType, null, this.urlTree.root, (-1), InheritedResolve.empty),
            ...
            return 0 === segmentGroup.segments.length && segmentGroup.hasChildren() ? this.processChildren(config, segmentGroup, inherited) : this.processSegment(config, segmentGroup, 0, segmentGroup.segments, inherited, outlet);
            ...
            var _a = match$1(rawSegment, route, segments, inherited.snapshot),
                ...
                snapshot = new ActivatedRouteSnapshot(consumedSegments, Object.freeze(merge(inherited.allParams, parameters)), Object.freeze(this.urlTree.queryParams), this.urlTree.fragment, merge(inherited.allData, getData(route)), outlet, route.component, route, getSourceSegmentGroup(rawSegment), getPathIndexShift(rawSegment) + consumedSegments.length, newInheritedResolve),
                newInherited = route.component ? InheritedFromParent.empty(snapshot) : new InheritedFromParent(inherited, snapshot, parameters, getData(route), newInheritedResolve);
            if (0 === slicedSegments.length && segmentGroup.hasChildren()) {
        ...
        Router = function() {
          ...
          return Router.prototype.resetRootComponentType = function(rootComponentType) {
            ...
            this.setUpLocationChangeListener(), this.navigateByUrl(this.location.path(!0), {replaceUrl: !0});
            ...
            var _b = void 0 === _a ? {} : _a,
                ...
                a = relativeTo ? relativeTo : this.routerState.root,
            ...
            if (void 0 === extras && (extras = {skipLocationChange: !1}), url instanceof UrlTree)
              return this.scheduleNavigation(url, extras);
            var urlTree = this.urlSerializer.parse(url);
            return this.scheduleNavigation(urlTree, extras);
          }, Router.prototype.navigate = function(commands, extras) {
            return void 0 === extras && (extras = {skipLocationChange: !1}), this.scheduleNavigation(this.createUrlTree(commands, extras), extras);
            ...
            return this.routerEvents.next(new NavigationStart(id, this.serializeUrl(url))), Promise.resolve().then(function(_) {
            ...
            return id !== this.navigationId ? (this.location.go(this.urlSerializer.serialize(this.currentUrlTree)), this.routerEvents.next(new NavigationCancel(id, this.serializeUrl(url), "Navigation ID " + id + " is not equal to the current navigation id " + this.navigationId)), Promise.resolve(!1)) : new Promise(function(resolvePromise, rejectPromise) {
              var state,
                  ...
                  redirectsApplied$ = applyRedirects(_this.injector, _this.configLoader, url, _this.config),
                  snapshot$ = rxjs_operator_mergeMap.mergeMap.call(redirectsApplied$, function(u) {
                    return appliedUrl = u, recognize(_this.rootComponentType, _this.config, appliedUrl, _this.serializeUrl(appliedUrl));
                  }),
                  emitRecognzied$ = rxjs_operator_map.map.call(snapshot$, function(newRouterStateSnapshot) {
                    return _this.routerEvents.next(new RoutesRecognized(id, _this.serializeUrl(url), _this.serializeUrl(appliedUrl), newRouterStateSnapshot)), newRouterStateSnapshot;
                  }),
                  ...
                  preactivation$ = rxjs_operator_map.map.call(routerState$, function(newState) {
                    state = newState, preActivation = new PreActivation(state.snapshot, _this.currentRouterState.snapshot, _this.injector), preActivation.traverse(_this.outletMap);
                  }),
                  preactivation2$ = rxjs_operator_mergeMap.mergeMap.call(preactivation$, function() {
                    return preActivation.checkGuards();
                  }),
                  resolveData$ = rxjs_operator_mergeMap.mergeMap.call(preactivation2$, function(shouldActivate) {
                    return shouldActivate ? rxjs_operator_map.map.call(preActivation.resolveData(), function() {
                      return shouldActivate;
                    }) : rxjs_observable_of.of(shouldActivate);
                  });
              resolveData$.forEach(function(shouldActivate) {
                if (!shouldActivate || id !== _this.navigationId)
                  return void(navigationIsSuccessful = !1);
                if (_this.currentUrlTree = appliedUrl, _this.currentRouterState = state, !shouldPreventPushState) {
                ...
                new ActivateRoutes(state, storedState).activate(_this.outletMap), navigationIsSuccessful = !0;
              }).then(function() {
                _this.navigated = !0, navigationIsSuccessful ? (_this.routerEvents.next(new NavigationEnd(id, _this.serializeUrl(url), _this.serializeUrl(appliedUrl))), resolvePromise(!0)) : (_this.routerEvents.next(new NavigationCancel(id, _this.serializeUrl(url), "")), resolvePromise(!1));
              }, function(e) {
                if (e instanceof NavigationCancelingError)
                  _this.navigated = !0, _this.routerEvents.next(new NavigationCancel(id, _this.serializeUrl(url), e.message)), resolvePromise(!1);
                else {
                ...
                id === _this.navigationId && (_this.currentRouterState = storedState, _this.currentUrlTree = storedUrl, _this.location.replaceState(_this.serializeUrl(storedUrl)));
              });
            });
          }, Router;
        }(),
        ...
        PreActivation = function() {
          function PreActivation(future, curr, injector) {
            this.future = future, this.curr = curr, this.injector = injector, this.checks = [];
          }
          return PreActivation.prototype.traverse = function(parentOutletMap) {
            ...
            var _this = this;
            if (0 === this.checks.length)
              return rxjs_observable_of.of(!0);
            var checks$ = rxjs_observable_from.from(this.checks),
                runningChecks$ = rxjs_operator_map.map.call(checks$, function(s) {
                ...
                mergedChecks$ = rxjs_operator_mergeAll.mergeAll.call(runningChecks$);
            ...
            if (0 === this.checks.length)
              return rxjs_observable_of.of(null);
            var checks$ = rxjs_observable_from.from(this.checks),
                runningChecks$ = rxjs_operator_concatMap.concatMap.call(checks$, function(s) {
                  return s instanceof CanActivate ? _this.runResolve(s.route) : rxjs_observable_of.of(null);
                });
            ...
            var obs = rxjs_operator_map.map.call(rxjs_observable_from.from(canActivate), function(c) {
              ...
              return wrapIntoObservable(guard.canActivate ? guard.canActivate(future, _this.future) : guard(future, _this.future));
            });
            return andObservables(obs);
            ...
            return andObservables(rxjs_operator_map.map.call(rxjs_observable_from.from(canActivateChildGuards), function(d) {
              var obs = rxjs_operator_map.map.call(rxjs_observable_from.from(d.guards), function(c) {
                ...
                return wrapIntoObservable(guard.canActivateChild ? guard.canActivateChild(future, _this.future) : guard(future, _this.future));
              });
              return andObservables(obs);
            }));
            ...
            var canDeactivate$ = rxjs_operator_map.map.call(rxjs_observable_from.from(canDeactivate), function(c) {
              ...
              return wrapIntoObservable(guard.canDeactivate ? guard.canDeactivate(component, curr, _this.curr) : guard(component, curr, _this.curr));
            }),
                merged$ = rxjs_operator_mergeAll.mergeAll.call(canDeactivate$);
            return rxjs_operator_every.every.call(merged$, function(result) {
              return result === !0;
            });
            ...
            return rxjs_operator_map.map.call(this.resolveNode(resolve.current, future), function(resolvedData) {
              return resolve.resolvedData = resolvedData, future.data = merge(future.data, resolve.flattenedResolvedData), null;
            });
            ...
            var config = closestLoadedConfig(snapshot),
                injector = config ? config.injector : this.injector;
            return injector.get(token);
          }, PreActivation;
        }(),
        ActivateRoutes = function() {
          function ActivateRoutes(futureState, currState) {
            this.futureState = futureState, this.currState = currState;
          }
          return ActivateRoutes.prototype.activate = function(parentOutletMap) {
            ...
            advanceActivatedRoute(this.futureState.root), this.activateChildRoutes(futureRoot, currRoot, parentOutletMap);
          }, ActivateRoutes.prototype.activateChildRoutes = function(futureNode, currNode, outletMap) {
            var _this = this,
                prevChildren = nodeChildrenAsMap(currNode);
            futureNode.children.forEach(function(c) {
              _this.activateRoutes(c, prevChildren[c.value.outlet], outletMap), delete prevChildren[c.value.outlet];
            }), forEach(prevChildren, function(v, k) {
            ...
            var resolved = [{
              ...
              useValue: outletMap
                ...
                loadedFactoryResolver = null,
                loadedInjector = null;
            config && (loadedFactoryResolver = config.factoryResolver, loadedInjector = config.injector, resolved.push({
            ...
            outlet && outlet.isActivated && (this.deactivateOutletMap(outlet.outletMap), outlet.deactivate());
        ...
        RouterLink = function() {
          function RouterLink(router, route, locationStrategy) {
            this.router = router, this.route = route, this.locationStrategy = locationStrategy, this.commands = [];
          }
          return Object.defineProperty(RouterLink.prototype, "routerLink", {
            set: function(data) {
              Array.isArray(data) ? this.commands = data : this.commands = [data];
            },
            enumerable: !0,
            configurable: !0
          }), RouterLink.prototype.onClick = function(button, ctrlKey, metaKey) {
            return !(0 === button && !ctrlKey && !metaKey) || (this.router.navigateByUrl(this.urlTree), !1);
          ...
          }], RouterLink.ctorParameters = [{type: Router}, {type: ActivatedRoute}, {type: _angular_common.LocationStrategy}], RouterLink.propDecorators = {
        ...
        RouterLinkWithHref = function() {
          ...
          return Object.defineProperty(RouterLinkWithHref.prototype, "routerLink", {
            ...
            return !(0 === button && !ctrlKey && !metaKey) || ("string" == typeof this.target && "_self" != this.target || (this.router.navigateByUrl(this.urlTree), !1));
          ...
          }], RouterLinkWithHref.ctorParameters = [{type: Router}, {type: ActivatedRoute}, {type: _angular_common.LocationStrategy}], RouterLinkWithHref.propDecorators = {
        ...
        RouterLinkActive = function() {
          function RouterLinkActive(router, element, renderer) {
            var _this = this;
            this.router = router, this.element = element, this.renderer = renderer, this.classes = [], this.routerLinkActiveOptions = {exact: !1}, this.subscription = router.events.subscribe(function(s) {
          ...
          return RouterLinkActive.prototype.ngAfterContentInit = function() {
            ...
            set: function(data) {
              Array.isArray(data) ? this.classes = data : this.classes = data.split(" ");
            },
            enumerable: !0,
            configurable: !0
          ...
          }], RouterLinkActive.ctorParameters = [{type: Router}, {type: _angular_core.ElementRef}, {type: _angular_core.Renderer}], RouterLinkActive.propDecorators = {
            links: [{
        ...
        RouterOutlet = function() {
          ...
          return RouterOutlet.prototype.ngOnDestroy = function() {
            ...
            if (this.activated) {
              ...
              this.activated.destroy(), this.activated = null, this.deactivateEvents.emit(c);
            }
            ...
            factory = loadedResolver ? loadedResolver.resolveComponentFactory(component) : this.resolver.resolveComponentFactory(component);
        ...
        RouterPreloader = function() {
          function RouterPreloader(router, moduleLoader, compiler, injector, preloadingStrategy) {
            this.router = router, this.injector = injector, this.preloadingStrategy = preloadingStrategy, this.loader = new RouterConfigLoader(moduleLoader, compiler);
          }
          return RouterPreloader.prototype.setUpPreloading = function() {
            ...
            for (var res = [],
              ...
              if (c.loadChildren && !c.canLoad && c._loadedConfig) {
                ...
                c.loadChildren && !c.canLoad ? res.push(this.preloadConfig(injector, c)) : c.children && res.push(this.processRoutes(injector, c.children));
            }
            ...
            return this.preloadingStrategy.preload(route, function() {
              ...
              return rxjs_operator_mergeMap.mergeMap.call(loaded, function(config) {
                var c = route;
                return c._loadedConfig = config, _this.processRoutes(config.injector, config.routes);
              });
            });
          }, RouterPreloader.decorators = [{type: _angular_core.Injectable}], RouterPreloader.ctorParameters = [{type: Router}, {type: _angular_core.NgModuleFactoryLoader}, {type: _angular_core.Compiler}, {type: _angular_core.Injector}, {type: PreloadingStrategy}], RouterPreloader;
        }(),
        ROUTER_DIRECTIVES = [RouterOutlet, RouterLink, RouterLinkWithHref, RouterLinkActive],
        ROUTER_CONFIGURATION = new _angular_core.OpaqueToken("ROUTER_CONFIGURATION"),
        ROUTER_FORROOT_GUARD = new _angular_core.OpaqueToken("ROUTER_FORROOT_GUARD"),

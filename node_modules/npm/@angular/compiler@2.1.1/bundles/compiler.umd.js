  ...
  (function(global, factory) {
    ...
    var TemplateParseVisitor = (function() {
      ...
      TemplateParseVisitor.prototype._parseInterpolation = function(value, sourceSpan) {
        ...
        try {
          ...
          if (isPresent(ast) && ast.ast.expressions.length > view_utils.MAX_INTERPOLATION_VALUES) {
        ...
        if (bindParts !== null) {
          ...
          if (isPresent(bindParts[KW_BIND_IDX])) {
            ...
            if (_isAnimationLabel(name) && isPresent(value) && value.length > 0) {
              this._reportError("Assigning animation triggers via @prop=\"exp\" attributes with an expression is invalid." + " Use property bindings (e.g. [@prop]=\"exp\") or use an attribute without a value (e.g. @prop) instead.", srcSpan, exports.ParseErrorLevel.FATAL);
            }
    ...
    var DirectiveWrapperCompiler = (function() {
      ...
      DirectiveWrapperCompiler.prototype.compile = function(dirMeta) {
        ...
        var fields = [new ClassField(CONTEXT_FIELD_NAME, importType(dirMeta.type)), new ClassField(CHANGED_FIELD_NAME, BOOL_TYPE)];
        ...
        Object.keys(dirMeta.inputs).forEach(function(inputFieldName, idx) {
          ...
          ctorStmts.push(THIS_EXPR.prop(fieldName).set(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))).toStmt());
        ...
        ctorStmts.push(THIS_EXPR.prop(CONTEXT_FIELD_NAME).set(importExpr(dirMeta.type).instantiate(dirDepParamNames.map(function(paramName) {
      ...
      DirectiveWrapperCompiler.ctorParameters = [{type: CompilerConfig}];
    ...
    function detectChangesInternalMethod(lifecycleHooks, logBindingUpdate) {
      ...
      return new ClassMethod('detectChangesInternal', [new FnParam(VIEW_VAR.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(RENDER_EL_VAR.name, DYNAMIC_TYPE), new FnParam(THROW_ON_CHANGE_VAR.name, BOOL_TYPE)], stmts, BOOL_TYPE);
    }
    function checkInputMethod(input, fieldExpr, lifecycleHooks) {
      ...
      if (lifecycleHooks.genChanges) {
        onChangeStatements.push(THIS_EXPR.prop(CHANGES_FIELD_NAME).key(literal(input)).set(importExpr(resolveIdentifier(Identifiers.SimpleChange)).instantiate([fieldExpr, CURR_VALUE_VAR])).toStmt());
      }
      ...
      boundProps.forEach(function(boundProp) {
        ...
        switch (boundProp.type) {
          ...
          case exports.PropertyBindingType.Animation:
            ...
            updateStmts.push(animationTransitionVar_1.set(animationFnExpr.callFn([THIS_EXPR, renderNode, oldRenderValue.equals(unitializedValue).conditional(emptyStateValue, oldRenderValue), renderValue.equals(unitializedValue).conditional(emptyStateValue, renderValue)])).toDeclStmt());
            detachStmts_1.push(animationTransitionVar_1.set(animationFnExpr.callFn([THIS_EXPR, renderNode, oldRenderValue, emptyStateValue])).toDeclStmt());
            eventListeners.forEach(function(listener) {
    ...
    function bindDirectiveInputs(directiveAst, directiveWrapperInstance, compileElement) {
      ...
      directiveAst.inputs.forEach(function(input) {
        ...
        detectChangesInInputsMethod.addStmt(directiveWrapperInstance.callMethod("check_" + input.directiveName, [currValExpr, DetectChangesVars.throwOnChange, evalResult.forceUpdate || literal(false)]).toStmt());
      });

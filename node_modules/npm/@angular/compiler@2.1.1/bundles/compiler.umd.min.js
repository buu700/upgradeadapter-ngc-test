...
(function(process) {
  !function(global, factory) {
    ...
    function _flattenArray(source, target) {
      if (isPresent(source))
        for (var i = 0; i < source.length; i++) {
          var item = source[i];
          Array.isArray(item) ? _flattenArray(item, target) : target.push(item);
        }
      return target;
    }
    ...
    function visitValue(value, visitor, context) {
      return Array.isArray(value) ? visitor.visitArray(value, context) : isStrictStringMap(value) ? visitor.visitStringMap(value, context) : isBlank(value) || isPrimitive(value) ? visitor.visitPrimitive(value, context) : visitor.visitOther(value, context);
    }
    ...
    function removeIdentifierDuplicates(items) {
      ...
      return items.forEach(function(item) {
        map.get(item.identifier.reference) || map.set(item.identifier.reference, item);
      }), MapWrapper.values(map);
    }
    function _normalizeArray(obj) {
      return obj || [];
    }
    function isStaticSymbol(value) {
      return "object" == typeof value && null !== value && value.name && value.filePath;
    }
    ...
    function assertInterpolationSymbols(identifier, value) {
      if (isPresent(value) && (!Array.isArray(value) || 2 != value.length))
    ...
    function sha1(str) {
      ...
      for (var sha1 = words32ToString([a, b, c, d, e]),
          hex = "",
          i = 0; i < sha1.length; i++) {
        var b_1 = sha1.charCodeAt(i);
        hex += (b_1 >>> 4 & 15).toString(16) + (15 & b_1).toString(16);
      }
    ...
    function utf8Encode(str) {
      for (var encoded = "",
        ...
        codePoint <= 127 ? encoded += String.fromCharCode(codePoint) : codePoint <= 2047 ? encoded += String.fromCharCode(192 | codePoint >>> 6, 128 | 63 & codePoint) : codePoint <= 65535 ? encoded += String.fromCharCode(224 | codePoint >>> 12, 128 | codePoint >>> 6 & 63, 128 | 63 & codePoint) : codePoint <= 2097151 && (encoded += String.fromCharCode(240 | codePoint >>> 18, 128 | codePoint >>> 12 & 63, 128 | codePoint >>> 6 & 63, 128 | 63 & codePoint));
      }
      return encoded;
    }
    function decodeSurrogatePairs(str, index) {
      ...
      return high >= 55296 && high <= 57343 && str.length > index + 1 && (low = str.charCodeAt(index + 1), low >= 56320 && low <= 57343) ? 1024 * (high - 55296) + low - 56320 + 65536 : high;
    }
    function stringToWords32(str) {
      for (var words32 = Array(str.length >>> 2),
          i = 0; i < words32.length; i++)
        words32[i] = 0;
      for (var i = 0; i < str.length; i++)
        words32[i >>> 2] |= (255 & str.charCodeAt(i)) << 8 * (3 - i & 3);
      return words32;
    }
    function words32ToString(words32) {
      for (var str = "",
          i = 0; i < 4 * words32.length; i++)
    ...
    function extractPlaceholders(messageBundle) {
      var messageMap = messageBundle.getMessageMap(),
          placeholders = {};
      return Object.keys(messageMap).forEach(function(msgId) {
    ...
    function _expandPluralForm(ast, errors) {
      var children = ast.cases.map(function(c) {
        ...
        return errors.push.apply(errors, expansionResult.errors), new Element("template", [new Attribute$1("ngPluralCase", "" + c.value, c.valueSourceSpan)], expansionResult.nodes, c.sourceSpan, c.sourceSpan, c.sourceSpan);
      }),
    ...
    function _expandDefaultForm(ast, errors) {
      var children = ast.cases.map(function(c) {
        var expansionResult = expandNodes(c.expression);
        return errors.push.apply(errors, expansionResult.errors), new Element("template", [new Attribute$1("ngSwitchCase", "" + c.value, c.valueSourceSpan)], expansionResult.nodes, c.sourceSpan, c.sourceSpan, c.sourceSpan);
      }),
    ...
    function _normalizeProviders(providers, sourceSpan, targetErrors, targetProviders) {
      return void 0 === targetProviders && (targetProviders = null), targetProviders || (targetProviders = []), isPresent(providers) && providers.forEach(function(provider) {
    ...
    function _resolveProviders(providers, providerType, eager, sourceSpan, targetErrors, targetProvidersByToken) {
      providers.forEach(function(provider) {
        ...
        if (isPresent(resolvedProvider) && resolvedProvider.multiProvider !== provider.multi && targetErrors.push(new ProviderError("Mixing multi and non multi provider is not possible for token " + resolvedProvider.token.name, sourceSpan)), resolvedProvider)
          ...
          resolvedProvider = new ProviderAst(provider.token, provider.multi, eager || lifecycleHooks.length > 0, [provider], providerType, lifecycleHooks, sourceSpan), targetProvidersByToken.set(provider.token.reference, resolvedProvider);
        }
      });
    }
    ...
    function _addQueryToTokenMap(map, query) {
      query.selectors.forEach(function(token) {
        ...
        entry || (entry = [], map.set(token.reference, entry)), entry.push(query);
      });
    }
    ...
    function extractStyleUrls(resolver, baseUrl, cssText) {
      var foundUrls = [],
          modifiedCssText = cssText.replace(_cssImportRe, function() {
            for (var m = [],
    ...
    function _isEndStateAnimateStep(step) {
      if (step instanceof AnimationStepAst && step.duration > 0 && 2 == step.keyframes.length) {
        ...
        return 0 === Object.keys(styles1).length && 0 === Object.keys(styles2).length;
      }
      return !1;
    }
    ...
    function _parseAnimationDeclarationStates(stateMetadata, errors) {
      ...
      stateMetadata.styles.styles.forEach(function(stylesEntry) {
        "object" == typeof stylesEntry && null !== stylesEntry ? styleValues.push(stylesEntry) : errors.push(new AnimationParseError("State based animations cannot contain references to other states"));
      });
    ...
    function _parseAnimationAlias(alias, errors) {
      switch (alias) {
        ...
        case ":leave":
          return "* => void";
        default:
          return errors.push(new AnimationParseError('the transition alias value "' + alias + '" is not supported')), "* => *";
      }
    }
    function _parseAnimationTransitionExpr(eventStr, errors) {
      var expressions = [];
      ":" == eventStr[0] && (eventStr = _parseAnimationAlias(eventStr, errors));
    ...
    function _normalizeStyleMetadata(entry, stateStyles, errors) {
      ...
      return entry.styles.forEach(function(styleEntry) {
        "string" == typeof styleEntry ? ListWrapper.addAll(normalizedStyles, _resolveStylesFromState(styleEntry, stateStyles, errors)) : normalizedStyles.push(styleEntry);
      }), normalizedStyles;
    }
    ...
    function _mergeAnimationStyles(stylesList, newItem) {
      if ("object" == typeof newItem && null !== newItem && stylesList.length > 0) {
        var lastIndex = stylesList.length - 1,
            lastItem = stylesList[lastIndex];
        if ("object" == typeof lastItem && null !== lastItem)
    ...
    function _resolveStylesFromState(stateName, stateStyles, errors) {
      ...
      if (":" != stateName[0])
        ...
        isPresent(value) ? value.styles.forEach(function(stylesEntry) {
          "object" == typeof stylesEntry && null !== stylesEntry && styles.push(stylesEntry);
    ...
    function _parseAnimationKeyframes(keyframeSequence, currentTime, collectedStyles, stateStyles, errors) {
      ...
      keyframeSequence.steps.forEach(function(styleMetadata) {
        ...
        styleMetadata.styles.forEach(function(entry) {
          Object.keys(entry).forEach(function(prop) {
            "offset" != prop && (keyframeStyles[prop] = entry[prop]);
          });
      ...
      for (var lastKeyframeStyles = lastKeyframe[1],
          i = 1; i <= limit; i++) {
      ...
      for (var _loop_1 = function(i) {
        var entry = rawKeyframes[i],
            styles = entry[1];
        Object.keys(styles).forEach(function(prop) {
          isPresent(lastKeyframeStyles[prop]) || (lastKeyframeStyles[prop] = styles[prop]);
        });
      },
          i = limit - 1; i >= 0; i--)
        _loop_1(i);
    ...
    function _parseTransitionAnimation(entry, currentTime, collectedStyles, stateStyles, errors) {
      ...
      if (entry instanceof CompileAnimationWithStepsMetadata) {
        ...
        if (entry.steps.forEach(function(entry) {
          ...
          currentTime += astDuration, playTime += astDuration, maxDuration = Math.max(astDuration, maxDuration), steps.push(innerAst);
    ...
    function _parseTimeExpression(exp, errors) {
      ...
      if ("string" == typeof exp) {
        ...
        "s" == durationUnit && (durationMatch *= _ONE_SECOND), duration = Math.floor(durationMatch);
        var delayMatch = matches[3],
            delayUnit = matches[4];
        if (isPresent(delayMatch)) {
          var delayVal = parseFloat(delayMatch);
          isPresent(delayUnit) && "s" == delayUnit && (delayVal *= _ONE_SECOND), delay = Math.floor(delayVal);
        }
    ...
    function detectChangesInternalMethod(lifecycleHooks, logBindingUpdate) {
      var changedVar = variable("changed"),
          stmts = [changedVar.set(THIS_EXPR.prop(CHANGED_FIELD_NAME)).toDeclStmt(), THIS_EXPR.prop(CHANGED_FIELD_NAME).set(literal(!1)).toStmt()],
          lifecycleStmts = [];
      if (lifecycleHooks.genChanges) {
        var onChangesStmts = [];
        lifecycleHooks.ngOnChanges && onChangesStmts.push(THIS_EXPR.prop(CONTEXT_FIELD_NAME).callMethod("ngOnChanges", [THIS_EXPR.prop(CHANGES_FIELD_NAME)]).toStmt()), logBindingUpdate && onChangesStmts.push(importExpr(resolveIdentifier(Identifiers.setBindingDebugInfoForChanges)).callFn([VIEW_VAR.prop("renderer"), RENDER_EL_VAR, THIS_EXPR.prop(CHANGES_FIELD_NAME)]).toStmt()), onChangesStmts.push(RESET_CHANGES_STMT), lifecycleStmts.push(new IfStmt(changedVar, onChangesStmts));
      }
      return lifecycleHooks.ngOnInit && lifecycleStmts.push(new IfStmt(VIEW_VAR.prop("numberOfChecks").identical(new LiteralExpr(0)), [THIS_EXPR.prop(CONTEXT_FIELD_NAME).callMethod("ngOnInit", []).toStmt()])), lifecycleHooks.ngDoCheck && lifecycleStmts.push(THIS_EXPR.prop(CONTEXT_FIELD_NAME).callMethod("ngDoCheck", []).toStmt()), lifecycleStmts.length > 0 && stmts.push(new IfStmt(not(THROW_ON_CHANGE_VAR), lifecycleStmts)), stmts.push(new ReturnStatement(changedVar)), new ClassMethod("detectChangesInternal", [new FnParam(VIEW_VAR.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(RENDER_EL_VAR.name, DYNAMIC_TYPE), new FnParam(THROW_ON_CHANGE_VAR.name, BOOL_TYPE)], stmts, BOOL_TYPE);
    }
    function checkInputMethod(input, fieldExpr, lifecycleHooks) {
      var onChangeStatements = [THIS_EXPR.prop(CHANGED_FIELD_NAME).set(literal(!0)).toStmt(), THIS_EXPR.prop(CONTEXT_FIELD_NAME).prop(input).set(CURR_VALUE_VAR).toStmt()];
      lifecycleHooks.genChanges && onChangeStatements.push(THIS_EXPR.prop(CHANGES_FIELD_NAME).key(literal(input)).set(importExpr(resolveIdentifier(Identifiers.SimpleChange)).instantiate([fieldExpr, CURR_VALUE_VAR])).toStmt()), onChangeStatements.push(fieldExpr.set(CURR_VALUE_VAR).toStmt());
      var methodBody = [new IfStmt(FORCE_UPDATE_VAR.or(importExpr(resolveIdentifier(Identifiers.checkBinding)).callFn([THROW_ON_CHANGE_VAR, fieldExpr, CURR_VALUE_VAR])), onChangeStatements)];
      return new ClassMethod("check_" + input, [new FnParam(CURR_VALUE_VAR.name, DYNAMIC_TYPE), new FnParam(THROW_ON_CHANGE_VAR.name, BOOL_TYPE), new FnParam(FORCE_UPDATE_VAR.name, BOOL_TYPE)], methodBody);
    }
    ...
    function createQueryList(query, directiveInstance, propertyName, compileView) {
      ...
      return compileView.createMethod.addStmt(THIS_EXPR.prop(propertyName).set(importExpr(resolveIdentifier(Identifiers.QueryList), [DYNAMIC_TYPE]).instantiate([])).toStmt()), expr;
    }
    function addQueryToTokenMap(map, query) {
      query.meta.selectors.forEach(function(selector) {
        var entry = map.get(selector.reference);
        entry || (entry = [], map.set(selector.reference, entry)), entry.push(query);
      });
    }
    ...
    function convertCdStatementToIr(nameResolver, implicitReceiver, stmt, bindingIndex) {
      var visitor = new _AstToIrVisitor(nameResolver, implicitReceiver, null, bindingIndex),
          statements = [];
      return flattenStatements(stmt.visit(visitor, _Mode.Statement), statements), prependTemporaryDecls(visitor.temporaryCount, bindingIndex, statements), statements;
    }
    ...
    function bindRenderOutputs(eventListeners) {
      eventListeners.forEach(function(listener) {
        listener.isAnimation || listener.listenToRenderer();
      });
    }
    ...
    function evalCdAst(view, currValExpr, parsedExpression, context, method, bindingIndex) {
      ...
      return method.addStmt(currValExpr.set(checkExpression.expression).toDeclStmt(null, [StmtModifier.Final])), new EvalResult(checkExpression.needsValueUnwrapper ? DetectChangesVars.valUnwrapper.prop("hasWrappedValue") : null);
    }
    function bind(view, currValExpr, fieldExpr, parsedExpression, context, actions, method, bindingIndex) {
      ...
      if (evalResult) {
        view.fields.push(new ClassField(fieldExpr.name, null, [StmtModifier.Private])), view.createMethod.addStmt(THIS_EXPR.prop(fieldExpr.name).set(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))).toStmt());
        ...
        evalResult.forceUpdate && (condition = evalResult.forceUpdate.or(condition)), method.addStmt(new IfStmt(condition, actions.concat([THIS_EXPR.prop(fieldExpr.name).set(currValExpr).toStmt()])));
      }
    }
    function bindRenderText(boundText, compileNode, view) {
      ...
      view.detectChangesRenderPropertiesMethod.resetDebugInfo(compileNode.nodeIndex, boundText), bind(view, currValExpr, valueField, boundText.value, view.componentContext, [THIS_EXPR.prop("renderer").callMethod("setText", [compileNode.renderNode, currValExpr]).toStmt()], view.detectChangesRenderPropertiesMethod, bindingIndex);
    }
    function bindAndWriteToRenderer(boundProps, context, compileElement, isHostProp, eventListeners) {
      ...
      boundProps.forEach(function(boundProp) {
        ...
        var fieldExpr = createBindFieldExpr(bindingIndex),
            ...
            updateStmts = [],
            compileMethod = view.detectChangesRenderPropertiesMethod;
        switch (boundProp.type) {
          ...
          case exports.PropertyBindingType.Animation:
            ...
            var detachStmts_1 = [],
                animationName_1 = boundProp.name,
                targetViewExpr = isHostProp ? compileElement.appElement.prop("componentView") : THIS_EXPR,
                animationFnExpr = targetViewExpr.prop("componentType").prop("animations").key(literal(animationName_1)),
                emptyStateValue = literal(EMPTY_ANIMATION_STATE),
                unitializedValue = importExpr(resolveIdentifier(Identifiers.UNINITIALIZED)),
                animationTransitionVar_1 = variable("animationTransition_" + animationName_1);
            updateStmts.push(animationTransitionVar_1.set(animationFnExpr.callFn([THIS_EXPR, renderNode, oldRenderValue.equals(unitializedValue).conditional(emptyStateValue, oldRenderValue), renderValue.equals(unitializedValue).conditional(emptyStateValue, renderValue)])).toDeclStmt()), detachStmts_1.push(animationTransitionVar_1.set(animationFnExpr.callFn([THIS_EXPR, renderNode, oldRenderValue, emptyStateValue])).toDeclStmt()), eventListeners.forEach(function(listener) {
              if (listener.isAnimation && listener.eventName === animationName_1) {
                ...
                updateStmts.push(animationStmt), detachStmts_1.push(animationStmt);
              }
    ...
    function sanitizedValue(boundProp, renderValue) {
      ...
      var ctx = ViewProperties.viewUtils.prop("sanitizer"),
          args = [importExpr(resolveIdentifier(Identifiers.SecurityContext)).prop(enumValue), renderValue];
      return ctx.callMethod("sanitize", args);
    }
    function bindRenderInputs(boundProps, compileElement, eventListeners) {
      bindAndWriteToRenderer(boundProps, compileElement.view.componentContext, compileElement, !1, eventListeners);
    }
    function bindDirectiveHostProps(directiveAst, directiveInstance, compileElement, eventListeners) {
      bindAndWriteToRenderer(directiveAst.hostProperties, directiveInstance, compileElement, !0, eventListeners);
    }
    function bindDirectiveInputs(directiveAst, directiveWrapperInstance, compileElement) {
      ...
      detectChangesInInputsMethod.resetDebugInfo(compileElement.nodeIndex, compileElement.sourceAst), directiveAst.inputs.forEach(function(input) {
        var bindingIndex = view.bindings.length;
        ...
        var currValExpr = createCurrValueExpr(bindingIndex),
            evalResult = evalCdAst(view, currValExpr, input.value, view.componentContext, detectChangesInInputsMethod, bindingIndex);
        evalResult && detectChangesInInputsMethod.addStmt(directiveWrapperInstance.callMethod("check_" + input.directiveName, [currValExpr, DetectChangesVars.throwOnChange, evalResult.forceUpdate || literal(!1)]).toStmt());
      });
      var isOnPushComp = directiveAst.directive.isComponent && !isDefaultChangeDetectionStrategy(directiveAst.directive.changeDetection),
          directiveDetectChangesExpr = directiveWrapperInstance.callMethod("detectChangesInternal", [THIS_EXPR, compileElement.renderNode, DetectChangesVars.throwOnChange]),
          directiveDetectChangesStmt = isOnPushComp ? new IfStmt(directiveDetectChangesExpr, [compileElement.appElement.prop("componentView").callMethod("markAsCheckOnce", []).toStmt()]) : directiveDetectChangesExpr.toStmt();
      detectChangesInInputsMethod.addStmt(directiveDetectChangesStmt);
    }
    function logBindingUpdateStmt(renderNode, propName, value) {
      return importExpr(resolveIdentifier(Identifiers.setBindingDebugInfo)).callFn([THIS_EXPR.prop("renderer"), renderNode, literal(propName), value]).toStmt();
    }
    ...
    function _mergeHtmlAndDirectiveAttrs(declaredHtmlAttrs, directives) {
      var result = {};
      return Object.keys(declaredHtmlAttrs).forEach(function(key) {
    ...
    function mapToKeyValueArray(data) {
      ...
      }), ListWrapper.sort(entryArray), entryArray;
    }
    function createViewTopLevelStmts(view, targetStatements) {
      ...
      0 === view.viewIndex && targetStatements.push(renderCompTypeVar.set(NULL_EXPR).toDeclStmt(importType(resolveIdentifier(Identifiers.RenderComponentType))));
    ...
    function createStaticNodeDebugInfo(node) {
      var compileElement = node instanceof CompileElement ? node : null,
          ...
          varTokenEntries = [];
      return isPresent(compileElement) && (providerTokens = compileElement.getProviderTokens(), isPresent(compileElement.component) && (componentToken = createDiTokenExpression(identifierToken(compileElement.component.type))), Object.keys(compileElement.referenceTokens).forEach(function(varName) {
    ...
    function createViewFactory(view, viewClass, renderCompTypeVar) {
      var templateUrlInfo,
          viewFactoryArgs = [new FnParam(ViewConstructorVars.viewUtils.name, importType(resolveIdentifier(Identifiers.ViewUtils))), new FnParam(ViewConstructorVars.parentInjector.name, importType(resolveIdentifier(Identifiers.Injector))), new FnParam(ViewConstructorVars.declarationEl.name, importType(resolveIdentifier(Identifiers.AppElement)))],
    ...
    function generateDetectChangesMethod(view) {
      ...
      if (view.animationBindingsMethod.isEmpty() && view.detectChangesInInputsMethod.isEmpty() && view.updateContentQueriesMethod.isEmpty() && view.afterContentLifecycleCallbacksMethod.isEmpty() && view.detectChangesRenderPropertiesMethod.isEmpty() && view.updateViewQueriesMethod.isEmpty() && view.afterViewLifecycleCallbacksMethod.isEmpty())
        return stmts;
      ListWrapper.addAll(stmts, view.animationBindingsMethod.finish()), ListWrapper.addAll(stmts, view.detectChangesInInputsMethod.finish()), stmts.push(THIS_EXPR.callMethod("detectContentChildrenChanges", [DetectChangesVars.throwOnChange]).toStmt());
      ...
      return readVars.has(DetectChangesVars.changed.name) && varStmts.push(DetectChangesVars.changed.set(literal(!0)).toDeclStmt(BOOL_TYPE)), readVars.has(DetectChangesVars.changes.name) && varStmts.push(DetectChangesVars.changes.set(NULL_EXPR).toDeclStmt(new MapType(importType(resolveIdentifier(Identifiers.SimpleChange))))), readVars.has(DetectChangesVars.valUnwrapper.name) && varStmts.push(DetectChangesVars.valUnwrapper.set(importExpr(resolveIdentifier(Identifiers.ValueUnwrapper)).instantiate([])).toDeclStmt(null, [StmtModifier.Final])), varStmts.concat(stmts);
    }
    ...
    function analyzeModules(ngModules, metadataResolver) {
      ...
      return ngModules.forEach(function(ngModule) {
        ...
        modules.push(ngModuleMeta), ngModuleMeta.declaredDirectives.forEach(function(dirMeta) {
          ngModuleByDirective.set(dirMeta.type.reference, ngModuleMeta);
        });
      }), new NgModulesSummary(ngModuleByDirective, modules);
    }
    ...
    function escapeIdentifier(input, escapeDollar, alwaysQuote) {
      if (void 0 === alwaysQuote && (alwaysQuote = !0), isBlank(input))
        return null;
      ...
      return TextAst.prototype.visit = function(visitor, context) {
        ...
        BoundEventAst = function() {
          ...
          return BoundEventAst.prototype.visit = function(visitor, context) {
            ...
            get: function() {
              return this.target ? this.target + ":" + this.name : this.name;
            },
            enumerable: !0,
            configurable: !0
        ...
        ElementAst = function() {
          function ElementAst(name, attrs, inputs, outputs, references, directives, providers, hasViewContainer, children, ngContentIndex, sourceSpan, endSourceSpan) {
            this.name = name, this.attrs = attrs, this.inputs = inputs, this.outputs = outputs, this.references = references, this.directives = directives, this.providers = providers, this.hasViewContainer = hasViewContainer, this.children = children, this.ngContentIndex = ngContentIndex, this.sourceSpan = sourceSpan, this.endSourceSpan = endSourceSpan;
          }
    ...
    var TagContentType,
        ...
        NumberWrapper = function() {
          ...
          return NumberWrapper.parseIntAutoRadix = function(text) {
        ...
        MapWrapper = function() {
          function MapWrapper() {}
          return MapWrapper.createFromStringMap = function(stringMap) {
            var result = new Map;
        ...
        StringMapWrapper = function() {
          ...
          return StringMapWrapper.merge = function(m1, m2) {
            for (var m = {},
                _i = 0,
    ...
    var TypeModifier,
        ...
        CssSelector = function() {
          ...
          return CssSelector.parse = function(selector) {
            ...
            for (_SELECTOR_REGEXP.lastIndex = 0; match = _SELECTOR_REGEXP.exec(selector); ) {
              ...
              if (match[2] && current.setElement(match[2]), match[3] && current.addClassName(match[3]), match[4] && current.addAttribute(match[4], match[5]), match[6] && (inNot = !1, current = cssSelector), match[7]) {
                if (inNot)
        ...
        SelectorMatcher = function() {
          function SelectorMatcher() {
            this._elementMap = {}, this._elementPartialMap = {}, this._classMap = {}, this._classPartialMap = {}, this._attrValueMap = {}, this._attrValuePartialMap = {}, this._listContexts = [];
          }
          return SelectorMatcher.createNotMatcher = function(notSelectors) {
              ...
              for (var i = 0; i < attrs.length; i += 2) {
                ...
                if (isTerminal) {
                  var terminalMap = matcher._attrValueMap,
                      terminalValuesMap = terminalMap[name_2];
                  ...
                  var partialMap = matcher._attrValuePartialMap,
                      partialValuesMap = partialMap[name_2];
                  partialValuesMap || (partialValuesMap = {}, partialMap[name_2] = partialValuesMap), matcher = this._addPartial(partialValuesMap, value);
                }
              }
          }, SelectorMatcher.prototype._addTerminal = function(map, name, selectable) {
            var terminalList = map[name];
            terminalList || (terminalList = [], map[name] = terminalList), terminalList.push(selectable);
          }, SelectorMatcher.prototype._addPartial = function(map, name) {
            var matcher = map[name];
            return matcher || (matcher = new SelectorMatcher, map[name] = matcher), matcher;
            ...
            if (attrs)
              for (var i = 0; i < attrs.length; i += 2) {
                var name_3 = attrs[i],
                    value = attrs[i + 1],
                    terminalValuesMap = this._attrValueMap[name_3];
            ...
            var selectables = map[name],
                starSelectables = map["*"];
            if (starSelectables && (selectables = selectables.concat(starSelectables)), !selectables)
              return !1;
            for (var selectable,
                result = !1,
        ...
        __extends$2 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
    ...
    var BinaryOperator,
        BuiltinType = function(_super) {
          ...
          return __extends$2(BuiltinType, _super), BuiltinType.prototype.visitType = function(visitor, context) {
        ...
        ExternalType = function(_super) {
          ...
          return __extends$2(ExternalType, _super), ExternalType.prototype.visitType = function(visitor, context) {
        ...
        ArrayType = function(_super) {
          ...
          return __extends$2(ArrayType, _super), ArrayType.prototype.visitType = function(visitor, context) {
        ...
        MapType = function(_super) {
          ...
          return __extends$2(MapType, _super), MapType.prototype.visitType = function(visitor, context) {
    ...
    var BuiltinMethod,
        ReadVarExpr = function(_super) {
          function ReadVarExpr(name, type) {
            void 0 === type && (type = null), _super.call(this, type), "string" == typeof name ? (this.name = name, this.builtin = null) : (this.name = null, this.builtin = name);
          }
          return __extends$2(ReadVarExpr, _super), ReadVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WriteVarExpr = function(_super) {
          function WriteVarExpr(name, value, type) {
            void 0 === type && (type = null), _super.call(this, type || value.type), this.name = name, this.value = value;
          }
          return __extends$2(WriteVarExpr, _super), WriteVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WriteKeyExpr = function(_super) {
          function WriteKeyExpr(receiver, index, value, type) {
            void 0 === type && (type = null), _super.call(this, type || value.type), this.receiver = receiver, this.index = index, this.value = value;
          }
          return __extends$2(WriteKeyExpr, _super), WriteKeyExpr.prototype.visitExpression = function(visitor, context) {
            return visitor.visitWriteKeyExpr(this, context);
          }, WriteKeyExpr;
        }(Expression),
        WritePropExpr = function(_super) {
          function WritePropExpr(receiver, name, value, type) {
            void 0 === type && (type = null), _super.call(this, type || value.type), this.receiver = receiver, this.name = name, this.value = value;
          }
          return __extends$2(WritePropExpr, _super), WritePropExpr.prototype.visitExpression = function(visitor, context) {
    ...
    var StmtModifier,
        InvokeMethodExpr = function(_super) {
          function InvokeMethodExpr(receiver, method, args, type) {
            void 0 === type && (type = null), _super.call(this, type), this.receiver = receiver, this.args = args, "string" == typeof method ? (this.name = method, this.builtin = null) : (this.name = null, this.builtin = method);
          }
          return __extends$2(InvokeMethodExpr, _super), InvokeMethodExpr.prototype.visitExpression = function(visitor, context) {
        ...
        InvokeFunctionExpr = function(_super) {
          ...
          return __extends$2(InvokeFunctionExpr, _super), InvokeFunctionExpr.prototype.visitExpression = function(visitor, context) {
        ...
        InstantiateExpr = function(_super) {
          ...
          return __extends$2(InstantiateExpr, _super), InstantiateExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralExpr = function(_super) {
          ...
          return __extends$2(LiteralExpr, _super), LiteralExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ExternalExpr = function(_super) {
          ...
          return __extends$2(ExternalExpr, _super), ExternalExpr.prototype.visitExpression = function(visitor, context) {
            return visitor.visitExternalExpr(this, context);
          }, ExternalExpr;
        }(Expression),
        ConditionalExpr = function(_super) {
          function ConditionalExpr(condition, trueCase, falseCase, type) {
            void 0 === falseCase && (falseCase = null), void 0 === type && (type = null), _super.call(this, type || trueCase.type), this.condition = condition, this.falseCase = falseCase, this.trueCase = trueCase;
          }
          return __extends$2(ConditionalExpr, _super), ConditionalExpr.prototype.visitExpression = function(visitor, context) {
        ...
        NotExpr = function(_super) {
          ...
          return __extends$2(NotExpr, _super), NotExpr.prototype.visitExpression = function(visitor, context) {
        ...
        CastExpr = function(_super) {
          ...
          return __extends$2(CastExpr, _super), CastExpr.prototype.visitExpression = function(visitor, context) {
        ...
        FunctionExpr = function(_super) {
          ...
          return __extends$2(FunctionExpr, _super), FunctionExpr.prototype.visitExpression = function(visitor, context) {
        ...
        BinaryOperatorExpr = function(_super) {
          function BinaryOperatorExpr(operator, lhs, rhs, type) {
            void 0 === type && (type = null), _super.call(this, type || lhs.type), this.operator = operator, this.rhs = rhs, this.lhs = lhs;
          }
          return __extends$2(BinaryOperatorExpr, _super), BinaryOperatorExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ReadPropExpr = function(_super) {
          ...
          return __extends$2(ReadPropExpr, _super), ReadPropExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ReadKeyExpr = function(_super) {
          ...
          return __extends$2(ReadKeyExpr, _super), ReadKeyExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralArrayExpr = function(_super) {
          ...
          return __extends$2(LiteralArrayExpr, _super), LiteralArrayExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralMapExpr = function(_super) {
          ...
          return __extends$2(LiteralMapExpr, _super), LiteralMapExpr.prototype.visitExpression = function(visitor, context) {
      ...
      return Statement.prototype.hasModifier = function(modifier) {
        ...
        DeclareVarStmt = function(_super) {
          function DeclareVarStmt(name, value, type, modifiers) {
            void 0 === type && (type = null), void 0 === modifiers && (modifiers = null), _super.call(this, modifiers), this.name = name, this.value = value, this.type = type || value.type;
          }
          return __extends$2(DeclareVarStmt, _super), DeclareVarStmt.prototype.visitStatement = function(visitor, context) {
        ...
        DeclareFunctionStmt = function(_super) {
          ...
          return __extends$2(DeclareFunctionStmt, _super), DeclareFunctionStmt.prototype.visitStatement = function(visitor, context) {
        ...
        ExpressionStatement = function(_super) {
          ...
          return __extends$2(ExpressionStatement, _super), ExpressionStatement.prototype.visitStatement = function(visitor, context) {
        ...
        ReturnStatement = function(_super) {
          ...
          return __extends$2(ReturnStatement, _super), ReturnStatement.prototype.visitStatement = function(visitor, context) {
        ...
        ClassField = function(_super) {
          ...
          return __extends$2(ClassField, _super), ClassField;
        }(AbstractClassPart),
        ClassMethod = function(_super) {
          ...
          return __extends$2(ClassMethod, _super), ClassMethod;
        }(AbstractClassPart),
        ClassGetter = function(_super) {
          ...
          return __extends$2(ClassGetter, _super), ClassGetter;
        }(AbstractClassPart),
        ClassStmt = function(_super) {
          ...
          return __extends$2(ClassStmt, _super), ClassStmt.prototype.visitStatement = function(visitor, context) {
        ...
        IfStmt = function(_super) {
          ...
          return __extends$2(IfStmt, _super), IfStmt.prototype.visitStatement = function(visitor, context) {
            return visitor.visitIfStmt(this, context);
          }, IfStmt;
        }(Statement),
        TryCatchStmt = (function(_super) {
          function CommentStmt(comment) {
            _super.call(this), this.comment = comment;
          }
          return __extends$2(CommentStmt, _super), CommentStmt.prototype.visitStatement = function(visitor, context) {
          ...
          return __extends$2(TryCatchStmt, _super), TryCatchStmt.prototype.visitStatement = function(visitor, context) {
        ...
        ThrowStmt = function(_super) {
          ...
          return __extends$2(ThrowStmt, _super), ThrowStmt.prototype.visitStatement = function(visitor, context) {
        ...
        _ReplaceVariableTransformer = function(_super) {
          ...
          return __extends$2(_ReplaceVariableTransformer, _super), _ReplaceVariableTransformer.prototype.visitReadVarExpr = function(ast, context) {
            return ast.name == this._varName ? this._newValue : ast;
          }, _ReplaceVariableTransformer;
        }(ExpressionTransformer),
        _VariableFinder = function(_super) {
          function _VariableFinder() {
            _super.apply(this, arguments), this.varNames = new Set;
          }
          return __extends$2(_VariableFinder, _super), _VariableFinder.prototype.visitReadVarExpr = function(ast, context) {
        ...
        __extends$1 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        CompileAnimationStateDeclarationMetadata = function(_super) {
          ...
          return __extends$1(CompileAnimationStateDeclarationMetadata, _super), CompileAnimationStateDeclarationMetadata;
        ...
        CompileAnimationStateTransitionMetadata = function(_super) {
          ...
          return __extends$1(CompileAnimationStateTransitionMetadata, _super), CompileAnimationStateTransitionMetadata;
        }(CompileAnimationStateMetadata),
        ...
        CompileAnimationKeyframesSequenceMetadata = function(_super) {
          ...
          return __extends$1(CompileAnimationKeyframesSequenceMetadata, _super), CompileAnimationKeyframesSequenceMetadata;
        ...
        CompileAnimationStyleMetadata = function(_super) {
          ...
          return __extends$1(CompileAnimationStyleMetadata, _super), CompileAnimationStyleMetadata;
        }(CompileAnimationMetadata),
        CompileAnimationAnimateMetadata = function(_super) {
          ...
          return __extends$1(CompileAnimationAnimateMetadata, _super), CompileAnimationAnimateMetadata;
        }(CompileAnimationMetadata),
        CompileAnimationWithStepsMetadata = function(_super) {
          function CompileAnimationWithStepsMetadata(steps) {
            void 0 === steps && (steps = null), _super.call(this), this.steps = steps;
          }
          return __extends$1(CompileAnimationWithStepsMetadata, _super), CompileAnimationWithStepsMetadata;
        }(CompileAnimationMetadata),
        CompileAnimationSequenceMetadata = function(_super) {
          function CompileAnimationSequenceMetadata(steps) {
            void 0 === steps && (steps = null), _super.call(this, steps);
          }
          return __extends$1(CompileAnimationSequenceMetadata, _super), CompileAnimationSequenceMetadata;
        }(CompileAnimationWithStepsMetadata),
        CompileAnimationGroupMetadata = function(_super) {
          function CompileAnimationGroupMetadata(steps) {
            void 0 === steps && (steps = null), _super.call(this, steps);
          }
          return __extends$1(CompileAnimationGroupMetadata, _super), CompileAnimationGroupMetadata;
        }(CompileAnimationWithStepsMetadata),
        CompileIdentifierMetadata = function() {
          function CompileIdentifierMetadata(_a) {
            var _b = void 0 === _a ? {} : _a,
                ...
                prefix = _b.prefix,
                value = _b.value;
            this.reference = reference, this.name = name, this.prefix = prefix, this.moduleUrl = moduleUrl, this.value = value;
          }
        ...
        CompileTokenMetadata = function() {
          ...
          return Object.defineProperty(CompileTokenMetadata.prototype, "reference", {
            get: function() {
              return isPresent(this.identifier) ? this.identifier.reference : this.value;
            },
            enumerable: !0,
            configurable: !0
        ...
        CompileTypeMetadata = function(_super) {
          ...
          return __extends$1(CompileTypeMetadata, _super), CompileTypeMetadata;
        }(CompileIdentifierMetadata),
        ...
        CompileTemplateMetadata = function() {
          function CompileTemplateMetadata(_a) {
            ...
            if (this.encapsulation = encapsulation, this.template = template, this.templateUrl = templateUrl, this.styles = _normalizeArray(styles), this.styleUrls = _normalizeArray(styleUrls), this.externalStylesheets = _normalizeArray(externalStylesheets), this.animations = isPresent(animations) ? ListWrapper.flatten(animations) : [], this.ngContentSelectors = ngContentSelectors || [], isPresent(interpolation) && 2 != interpolation.length)
        ...
        CompileDirectiveMetadata = function() {
          ...
          return CompileDirectiveMetadata.create = function(_a) {
            var _b = void 0 === _a ? {} : _a,
                ...
                hostProperties = {},
                hostAttributes = {};
            isPresent(host) && Object.keys(host).forEach(function(key) {
              var value = host[key],
                  matches = key.match(HOST_REG_EXP);
              null === matches ? hostAttributes[key] = value : isPresent(matches[1]) ? hostProperties[matches[1]] = value : isPresent(matches[2]) && (hostListeners[matches[2]] = value);
            });
            var inputsMap = {};
        ...
        __extends$3 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        Quote = function(_super) {
          ...
          return __extends$3(Quote, _super), Quote.prototype.visit = function(visitor, context) {
        ...
        EmptyExpr = function(_super) {
          ...
          return __extends$3(EmptyExpr, _super), EmptyExpr.prototype.visit = function(visitor, context) {
            void 0 === context && (context = null);
          }, EmptyExpr;
        }(AST),
        ImplicitReceiver = function(_super) {
          ...
          return __extends$3(ImplicitReceiver, _super), ImplicitReceiver.prototype.visit = function(visitor, context) {
        ...
        Chain = function(_super) {
          ...
          return __extends$3(Chain, _super), Chain.prototype.visit = function(visitor, context) {
        ...
        Conditional = function(_super) {
          ...
          return __extends$3(Conditional, _super), Conditional.prototype.visit = function(visitor, context) {
        ...
        PropertyRead = function(_super) {
          ...
          return __extends$3(PropertyRead, _super), PropertyRead.prototype.visit = function(visitor, context) {
        ...
        PropertyWrite = function(_super) {
          ...
          return __extends$3(PropertyWrite, _super), PropertyWrite.prototype.visit = function(visitor, context) {
        ...
        SafePropertyRead = function(_super) {
          ...
          return __extends$3(SafePropertyRead, _super), SafePropertyRead.prototype.visit = function(visitor, context) {
        ...
        KeyedRead = function(_super) {
          ...
          return __extends$3(KeyedRead, _super), KeyedRead.prototype.visit = function(visitor, context) {
        ...
        KeyedWrite = function(_super) {
          ...
          return __extends$3(KeyedWrite, _super), KeyedWrite.prototype.visit = function(visitor, context) {
        ...
        BindingPipe = function(_super) {
          ...
          return __extends$3(BindingPipe, _super), BindingPipe.prototype.visit = function(visitor, context) {
        ...
        LiteralPrimitive = function(_super) {
          ...
          return __extends$3(LiteralPrimitive, _super), LiteralPrimitive.prototype.visit = function(visitor, context) {
        ...
        LiteralArray = function(_super) {
          ...
          return __extends$3(LiteralArray, _super), LiteralArray.prototype.visit = function(visitor, context) {
        ...
        LiteralMap = function(_super) {
          ...
          return __extends$3(LiteralMap, _super), LiteralMap.prototype.visit = function(visitor, context) {
        ...
        Interpolation = function(_super) {
          ...
          return __extends$3(Interpolation, _super), Interpolation.prototype.visit = function(visitor, context) {
        ...
        Binary = function(_super) {
          ...
          return __extends$3(Binary, _super), Binary.prototype.visit = function(visitor, context) {
        ...
        PrefixNot = function(_super) {
          ...
          return __extends$3(PrefixNot, _super), PrefixNot.prototype.visit = function(visitor, context) {
        ...
        MethodCall = function(_super) {
          ...
          return __extends$3(MethodCall, _super), MethodCall.prototype.visit = function(visitor, context) {
        ...
        SafeMethodCall = function(_super) {
          ...
          return __extends$3(SafeMethodCall, _super), SafeMethodCall.prototype.visit = function(visitor, context) {
        ...
        FunctionCall = function(_super) {
          ...
          return __extends$3(FunctionCall, _super), FunctionCall.prototype.visit = function(visitor, context) {
        ...
        ASTWithSource = function(_super) {
          ...
          return __extends$3(ASTWithSource, _super), ASTWithSource.prototype.visit = function(visitor, context) {
    ...
    exports.TokenType, function(TokenType) {
    ...
    var KEYWORDS = ["var", "let", "null", "undefined", "true", "false", "if", "else", "this"],
        Lexer = function() {
          ...
          return Lexer.prototype.tokenize = function(text) {
            ...
            return tokens;
          }, Lexer.decorators = [{type: _angular_core.Injectable}], Lexer.ctorParameters = [], Lexer;
        }(),
        Token = function() {
          ...
          return Token.prototype.isCharacter = function(code) {
            ...
            return this.type == exports.TokenType.Keyword && "let" == this.strValue;
          }, Token.prototype.isKeywordNull = function() {
            return this.type == exports.TokenType.Keyword && "null" == this.strValue;
          }, Token.prototype.isKeywordUndefined = function() {
            return this.type == exports.TokenType.Keyword && "undefined" == this.strValue;
          }, Token.prototype.isKeywordTrue = function() {
            return this.type == exports.TokenType.Keyword && "true" == this.strValue;
          }, Token.prototype.isKeywordFalse = function() {
            return this.type == exports.TokenType.Keyword && "false" == this.strValue;
          }, Token.prototype.isKeywordThis = function() {
            return this.type == exports.TokenType.Keyword && "this" == this.strValue;
        ...
        EOF = new Token((-1), exports.TokenType.Character, 0, ""),
        _Scanner = function() {
          ...
          return _Scanner.prototype.advance = function() {
            ...
            var str = this.input.substring(start, this.index),
                value = simple ? NumberWrapper.parseIntAutoRadix(str) : parseFloat(str);
        ...
        Parser = function() {
          ...
          }, Parser.decorators = [{type: _angular_core.Injectable}], Parser.ctorParameters = [{type: Lexer}], Parser;
        }(),
        _ParseAST = function() {
          ...
          return _ParseAST.prototype.peek = function(offset) {
            ...
            for (var result = this.parseRelational(); this.next.type == exports.TokenType.Operator; ) {
            ...
            for (var result = this.parseAdditive(); this.next.type == exports.TokenType.Operator; ) {
              var operator = this.next.strValue;
              switch (operator) {
                case "<":
                case ">":
            ...
            for (var result = this.parseMultiplicative(); this.next.type == exports.TokenType.Operator; ) {
              var operator = this.next.strValue;
              switch (operator) {
                case "+":
                case "-":
            ...
            for (var result = this.parsePrefix(); this.next.type == exports.TokenType.Operator; ) {
              var operator = this.next.strValue;
              switch (operator) {
                case "*":
                case "%":
 ...
 while (operatorFound);
    ...
    exports.ParseErrorLevel, function(ParseErrorLevel) {
      ParseErrorLevel[ParseErrorLevel.WARNING = 0] = "WARNING", ParseErrorLevel[ParseErrorLevel.FATAL = 1] = "FATAL";
    }(exports.ParseErrorLevel || (exports.ParseErrorLevel = {}));
    var TokenType$1,
        ParseError = function() {
          function ParseError(span, msg, level) {
            void 0 === level && (level = exports.ParseErrorLevel.FATAL), this.span = span, this.msg = msg, this.level = level;
          }
        ...
        __extends$5 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
    ...
    var _VisitorMode,
        ...
        TokenError = function(_super) {
          ...
          return __extends$5(TokenError, _super), TokenError;
        }(ParseError),
 ...
 while (!this._isTextEnd());
        ...
        __extends$4 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        TreeError = function(_super) {
          ...
          return __extends$4(TreeError, _super), TreeError.create = function(elementName, span, msg) {
        ...
        Message = function() {
          function Message(nodes, placeholders, placeholderToMsgIds, meaning, description) {
            this.nodes = nodes, this.placeholders = placeholders, this.placeholderToMsgIds = placeholderToMsgIds, this.meaning = meaning, this.description = description;
          }
          return Message;
        }(),
        Text$1 = function() {
        ...
        _I18nVisitor = function() {
          ...
          return _I18nVisitor.prototype.toI18nMessage = function(nodes, meaning, description) {
            this._isIcu = 1 == nodes.length && nodes[0] instanceof Expansion, this._icuDepth = 0, this._placeholderRegistry = new PlaceholderRegistry, this._placeholderToContent = {}, this._placeholderToIds = {};
            ...
            return this._placeholderToIds[phName] = digestMessage(visitor.toI18nMessage([icu], "", "")), new IcuPlaceholder(i18nIcu, phName, icu.sourceSpan);
        ...
        __extends$6 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        I18nError = function(_super) {
          ...
          return __extends$6(I18nError, _super), I18nError;
        }(ParseError),
        _I18N_ATTR = "i18n",
    ...
    var PreparsedElementType,
        _Visitor = function() {
          ...
          return _Visitor.prototype.extract = function(nodes, interpolationConfig) {
            ...
            if (this._inImplicitNode = this._inImplicitNode || isImplicit, this._isInTranslatableSection || this._inIcu)
              (i18nAttr || isTopLevelImplicit) && this._reportError(el, "Could not mark an element as translatable inside a translatable section"), this._mode == _VisitorMode.Extract && visitAll(this, el.children), this._mode == _VisitorMode.Merge && (childNodes = [], el.children.forEach(function(child) {
              ...
              this._mode !== _VisitorMode.Merge || i18nAttr || isTopLevelImplicit || (childNodes = [], el.children.forEach(function(child) {
                var visited = child.visit(_this, context);
                visited && !_this._isInTranslatableSection && (childNodes = childNodes.concat(visited));
              }));
            }
        ...
        __extends$7 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        XmlParser = function(_super) {
          ...
          return __extends$7(XmlParser, _super), XmlParser.prototype.parse = function(source, url, parseExpansionForms) {
        ...
        __extends$8 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        CR = function(_super) {
          function CR(ws) {
            void 0 === ws && (ws = 0), _super.call(this, "\n" + new Array(ws + 1).join(" "));
          }
          return __extends$8(CR, _super), CR;
        }(Text$2),
        ...
        Xliff = function() {
          ...
          return Xliff.prototype.write = function(messageMap) {
            var visitor = new _WriteVisitor,
                transUnits = [];
            Object.keys(messageMap).forEach(function(id) {
              var message = messageMap[id],
                  transUnit = new Tag(_UNIT_TAG, {
                    id: id,
                    datatype: "html"
                  });
        ...
        _WriteVisitor = function() {
          ...
          return _WriteVisitor.prototype.visitText = function(text, context) {
            ...
            var _this = this;
            return this._isInIcu = !1, ListWrapper.flatten(nodes.map(function(node) {
              return node.visit(_this);
            }));
          }, _WriteVisitor;
        }(),
        _LoadVisitor = function() {
          ...
          return _LoadVisitor.prototype.parse = function(nodes, messageBundle) {
            var _this = this;
            this._messageNodes = [], this._translatedMessages = {}, this._msgId = "", this._target = [], this._errors = [], visitAll(this, nodes, null);
            var messageMap = messageBundle.getMessageMap(),
                placeholders = extractPlaceholders(messageBundle),
                placeholderToIds = extractPlaceholderToIds(messageBundle);
            return this._messageNodes.filter(function(message) {
              return messageMap.hasOwnProperty(message[0]);
            }).sort(function(a, b) {
              return 0 == Object.keys(messageMap[a[0]].placeholderToMsgIds).length ? -1 : 0 == Object.keys(messageMap[b[0]].placeholderToMsgIds).length ? 1 : 0;
            }).forEach(function(message) {
              var id = message[0];
              _this._placeholders = placeholders[id] || {}, _this._placeholderToIds = placeholderToIds[id] || {}, _this._translatedMessages[id] = visitAll(_this, message[1]).join("");
            }), {
            ...
            switch (element.name) {
              case _UNIT_TAG:
                ...
                msgId ? this._msgId = msgId.value : this._addError(element, "<" + _UNIT_TAG + '> misses the "id" attribute'), visitAll(this, element.children, null), null !== this._msgId && this._messageNodes.push([this._msgId, this._target]);
                break;
              case _SOURCE_TAG:
                break;
              case _TARGET_TAG:
        ...
        Xmb = function() {
          ...
          return Xmb.prototype.write = function(messageMap) {
            ...
            return Object.keys(messageMap).forEach(function(id) {
              var message = messageMap[id],
                  attrs = {id: id};
        ...
        _Visitor$3 = function() {
          function _Visitor() {}
          return _Visitor.prototype.parse = function(nodes, messageBundle) {
            var _this = this;
            this._messageNodes = [], this._translatedMessages = {}, this._bundleDepth = 0, this._translationDepth = 0, this._errors = [], visitAll(this, nodes, null);
            var messageMap = messageBundle.getMessageMap(),
                placeholders = extractPlaceholders(messageBundle),
                placeholderToIds = extractPlaceholderToIds(messageBundle);
            return this._messageNodes.filter(function(message) {
            ...
            switch (element.name) {
              ...
              case _TRANSLATION_TAG:
                ...
                idAttr ? this._messageNodes.push([idAttr.value, element.children]) : this._addError(element, "<" + _TRANSLATION_TAG + '> misses the "id" attribute'), this._translationDepth--;
                break;
        ...
        TranslationBundle = function() {
          function TranslationBundle(_messageMap) {
            void 0 === _messageMap && (_messageMap = {}), this._messageMap = _messageMap;
          }
          return TranslationBundle.load = function(content, url, messageBundle, serializer) {
        ...
        I18NHtmlParser = function() {
          ...
          return I18NHtmlParser.prototype.parse = function(source, url, parseExpansionForms, interpolationConfig) {
            ...
            var messageBundle = new MessageBundle(this._htmlParser, [], {}),
                errors = messageBundle.updateFromTemplate(source, url, interpolationConfig);
            ...
            var serializer = this._createSerializer(interpolationConfig),
                translationBundle = TranslationBundle.load(this._translations, url, messageBundle, serializer);
        ...
        registerModuleFactory = _angular_core.__core_private__.registerModuleFactory,
        ViewType = _angular_core.__core_private__.ViewType,
        view_utils = _angular_core.__core_private__.view_utils,
        ...
        AnimationSequencePlayer = _angular_core.__core_private__.AnimationSequencePlayer,
        AnimationGroupPlayer = _angular_core.__core_private__.AnimationGroupPlayer,
        AnimationKeyframe = _angular_core.__core_private__.AnimationKeyframe,
        AnimationStyles = _angular_core.__core_private__.AnimationStyles,
        ...
        prepareFinalAnimationStyles = _angular_core.__core_private__.prepareFinalAnimationStyles,
        balanceAnimationKeyframes = _angular_core.__core_private__.balanceAnimationKeyframes,
        clearStyles = _angular_core.__core_private__.clearStyles,
        collectAndResolveStyles = _angular_core.__core_private__.collectAndResolveStyles,
        renderStyles = _angular_core.__core_private__.renderStyles,
        ComponentStillLoadingError = _angular_core.__core_private__.ComponentStillLoadingError,
        AnimationTransition = _angular_core.__core_private__.AnimationTransition,
        ...
        __extends$9 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        HtmlParser = function(_super) {
          ...
          return __extends$9(HtmlParser, _super), HtmlParser.prototype.parse = function(source, url, parseExpansionForms, interpolationConfig) {
          ...
          }, HtmlParser.decorators = [{type: _angular_core.Injectable}], HtmlParser.ctorParameters = [], HtmlParser;
        }(Parser$1),
        __extends$10 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        ExpansionError = function(_super) {
          ...
          return __extends$10(ExpansionError, _super), ExpansionError;
        }(ParseError),
        _Expander = function() {
          function _Expander() {
        ...
        __extends$11 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        ProviderError = function(_super) {
          ...
          return __extends$11(ProviderError, _super), ProviderError;
        }(ParseError),
        ProviderViewContext = function() {
          function ProviderViewContext(component, sourceSpan) {
            var _this = this;
            this.component = component, this.sourceSpan = sourceSpan, this.errors = [], this.viewQueries = _getViewQueries(component), this.viewProviders = new Map, _normalizeProviders(component.viewProviders, sourceSpan, this.errors).forEach(function(provider) {
              isBlank(_this.viewProviders.get(provider.token.reference)) && _this.viewProviders.set(provider.token.reference, !0);
            });
          }
        ...
        ProviderElementContext = function() {
          function ProviderElementContext(viewContext, _parent, _isViewRoot, _directiveAsts, attrs, refs, _sourceSpan) {
            var _this = this;
            this.viewContext = viewContext, this._parent = _parent, this._isViewRoot = _isViewRoot, this._directiveAsts = _directiveAsts, this._sourceSpan = _sourceSpan, this._transformedProviders = new Map, this._seenProviders = new Map, this._hasViewContainer = !1, this._attrs = {}, attrs.forEach(function(attrAst) {
            ...
            this._allProviders = _resolveProvidersFromDirectives(directivesMeta, _sourceSpan, viewContext.errors), this._contentQueries = _getContentQueries(directivesMeta);
            ...
            }), isPresent(queriedTokens.get(resolveIdentifierToken(Identifiers.ViewContainerRef).reference)) && (this._hasViewContainer = !0), MapWrapper.values(this._allProviders).forEach(function(provider) {
          ...
          return ProviderElementContext.prototype.afterElement = function() {
            ...
            this._getQueriesFor(token).forEach(function(query) {
              ...
              isBlank(queryReadTokens.get(queryReadToken.reference)) && queryReadTokens.set(queryReadToken.reference, !0);
            });
            ...
            for (var queries,
              ...
              queries = currentEl._contentQueries.get(token.reference), isPresent(queries) && ListWrapper.addAll(result, queries.filter(function(query) {
                return query.descendants || distance <= 1;
              })), currentEl._directiveAsts.length > 0 && distance++, currentEl = currentEl._parent;
            return queries = this.viewContext.viewQueries.get(token.reference), isPresent(queries) && ListWrapper.addAll(result, queries), result;
          }, ProviderElementContext.prototype._getOrCreateLocalProvider = function(requestingProviderType, token, eager) {
            var _this = this,
                resolvedProvider = this._allProviders.get(token.reference);
            ...
            if (isPresent(this._seenProviders.get(token.reference)))
              return this.viewContext.errors.push(new ProviderError("Cannot instantiate cyclic dependency! " + token.name, this._sourceSpan)), null;
            this._seenProviders.set(token.reference, !0);
            ...
            }), this._transformedProviders.set(token.reference, transformedProviderAst), transformedProviderAst;
            ...
            if (isPresent(dep.token)) {
              if (requestingProviderType === exports.ProviderAstType.Directive || requestingProviderType === exports.ProviderAstType.Component) {
                if (dep.token.reference === resolveIdentifierToken(Identifiers.Renderer).reference || dep.token.reference === resolveIdentifierToken(Identifiers.ElementRef).reference || dep.token.reference === resolveIdentifierToken(Identifiers.ChangeDetectorRef).reference || dep.token.reference === resolveIdentifierToken(Identifiers.TemplateRef).reference)
                  return dep;
                dep.token.reference === resolveIdentifierToken(Identifiers.ViewContainerRef).reference && (this._hasViewContainer = !0);
              }
            ...
            if (dep.isSkipSelf || (result = this._getLocalDependency(requestingProviderType, dep, eager)), dep.isSelf)
              ...
              for (; !result && isPresent(currElement._parent); ) {
              ...
              result || (result = !dep.isHost || this.viewContext.component.type.isHost || this.viewContext.component.type.reference === dep.token.reference || isPresent(this.viewContext.viewProviders.get(dep.token.reference)) ? dep : dep.isOptional ? result = new CompileDiDependencyMetadata({
                isValue: !0,
                value: null
              }) : null);
            }
            return result || this.viewContext.errors.push(new ProviderError("No provider for " + dep.token.name, this._sourceSpan)), result;
          }, ProviderElementContext;
        }(),
        NgModuleProviderAnalyzer = function() {
          function NgModuleProviderAnalyzer(ngModule, extraProviders, sourceSpan) {
            var _this = this;
            this._transformedProviders = new Map, this._seenProviders = new Map, this._errors = [], this._allProviders = new Map;
          ...
          return NgModuleProviderAnalyzer.prototype.parse = function() {
            var _this = this;
            if (MapWrapper.values(this._allProviders).forEach(function(provider) {
            ...
            void 0 === eager && (eager = null);
            var foundLocal = !1;
            !dep.isSkipSelf && isPresent(dep.token) && (dep.token.reference === resolveIdentifierToken(Identifiers.Injector).reference || dep.token.reference === resolveIdentifierToken(Identifiers.ComponentFactoryResolver).reference ? foundLocal = !0 : isPresent(this._getOrCreateLocalProvider(dep.token, eager)) && (foundLocal = !0));
            var result = dep;
    ...
    var _Mode,
        ...
        TemplateParser = function() {
          ...
          }, TemplateParser.decorators = [{type: _angular_core.Injectable}], TemplateParser.ctorParameters = [{type: Parser}, {type: ElementSchemaRegistry}, {type: I18NHtmlParser}, {type: Console}, {
            type: Array,
        ...
        TemplateParseVisitor = function() {
          function TemplateParseVisitor(providerViewContext, directives, pipes, _schemas, _exprParser, _schemaRegistry) {
            var _this = this;
            this.providerViewContext = providerViewContext, this._schemas = _schemas, this._exprParser = _exprParser, this._schemaRegistry = _schemaRegistry, this.selectorMatcher = new SelectorMatcher, this.errors = [], this.directivesIndex = new Map, this.ngContentCount = 0, this.pipesByName = new Map;
            var tempMeta = providerViewContext.component.template;
            tempMeta && tempMeta.interpolation && (this._interpolationConfig = {
          ...
          return TemplateParseVisitor.prototype._reportError = function(message, sourceSpan, level) {
            void 0 === level && (level = exports.ParseErrorLevel.FATAL), this.errors.push(new TemplateParseError(message, sourceSpan, level));
            ...
            try {
              ...
              if (ast && this._reportParserErrors(ast.errors, sourceSpan), this._checkPipes(ast, sourceSpan), isPresent(ast) && ast.ast.expressions.length > view_utils.MAX_INTERPOLATION_VALUES)
            ...
            try {
              ...
              return ast && this._reportParserErrors(ast.errors, sourceSpan), !ast || ast.ast instanceof EmptyExpr ? (this._reportError("Empty expressions are not allowed", sourceSpan), this._exprParser.wrapLiteralPrimitive("ERROR", sourceInfo)) : (this._checkPipes(ast, sourceSpan), ast);
            } catch (e) {
              return this._reportError("" + e, sourceSpan), this._exprParser.wrapLiteralPrimitive("ERROR", sourceInfo);
            }
            ...
            try {
              ...
              return ast && this._reportParserErrors(ast.errors, sourceSpan), this._checkPipes(ast, sourceSpan), ast;
            } catch (e) {
              return this._reportError("" + e, sourceSpan), this._exprParser.wrapLiteralPrimitive("ERROR", sourceInfo);
            }
            ...
            try {
              ...
              return this._reportParserErrors(bindingsResult.errors, sourceSpan), bindingsResult.templateBindings.forEach(function(binding) {
            ...
            if (preparsedElement.type === PreparsedElementType.NG_CONTENT)
              ...
              parsedElement = new ElementAst(nodeName, attrs, elementProps, events, references, providerContext.transformedDirectiveAsts, providerContext.transformProviders, providerContext.transformedHasViewContainer, children, hasInlineTemplates ? null : ngContentIndex_1, element.sourceSpan, element.endSourceSpan), this._findComponentDirectives(directiveAsts).forEach(function(componentDirectiveAst) {
            ...
            if (hasInlineTemplates) {
              var templateCssSelector = createElementCssSelector(TEMPLATE_ELEMENT, templateMatchableAttrs),
                  templateDirectiveMetas = this._parseDirectives(this.selectorMatcher, templateCssSelector).directives,
            ...
            animationInputs.forEach(function(input) {
              var name = input.name;
              triggerLookup.has(name) || _this._reportError("Couldn't find an animation entry for \"" + name + '"', input.sourceSpan);
            }), outputs.forEach(function(output) {
              if (output.isAnimation) {
                var found = animationInputs.find(function(input) {
                  return input.name == output.name;
                });
                found || _this._reportError("Unable to listen on (@" + output.name + "." + output.phase + ") because the animation trigger [@" + output.name + "] isn't being used on the same element", output.sourceSpan);
              }
            });
            ...
            var name = this._normalizeAttributeName(attr.name),
                ...
                hasBinding = !1;
            if (null !== bindParts)
              if (hasBinding = !0, isPresent(bindParts[KW_BIND_IDX]))
            ...
            var animatePropLength = ANIMATE_PROP_PREFIX.length,
                isAnimationProp = _isAnimationLabel(name),
                animationPrefixLength = 1;
            name.substring(0, animatePropLength) == ANIMATE_PROP_PREFIX && (isAnimationProp = !0, animationPrefixLength = animatePropLength), isAnimationProp ? this._parseAnimation(name.substr(animationPrefixLength), expression, sourceSpan, targetMatchableAttrs, targetAnimationProps) : this._parsePropertyAst(name, this._parseBinding(expression, sourceSpan), sourceSpan, targetMatchableAttrs, targetProps);
            ...
            return elementOrDirectiveRefs.forEach(function(elOrDirRef) {
              if (elOrDirRef.value.length > 0)
                ...
                var refToken = null;
                isTemplateElement && (refToken = resolveIdentifierToken(Identifiers.TemplateRef)), targetReferences.push(new ReferenceAst(elOrDirRef.name, refToken, elOrDirRef.sourceSpan));
              }
            }), directiveAsts;
          }, TemplateParseVisitor.prototype._createDirectiveHostPropertyAsts = function(elementName, hostProps, sourceSpan, targetPropertyAsts) {
            var _this = this;
            hostProps && Object.keys(hostProps).forEach(function(propName) {
            ...
            var _this = this;
            hostListeners && Object.keys(hostListeners).forEach(function(propName) {
              var expression = hostListeners[propName];
              "string" == typeof expression ? _this._parseEventOrAnimationEvent(propName, expression, sourceSpan, [], targetEventAsts) : _this._reportError('Value of the host listener "' + propName + '" needs to be a string representing an expression but got "' + expression + '" (' + typeof expression + ")", sourceSpan);
            });
            ...
            if (directiveProperties) {
              ...
              boundProps.forEach(function(boundProp) {
                ...
                boundProp && targetBoundDirectiveProps.push(new BoundDirectivePropertyAst(dirProp, boundProp.name, boundProp.expression, boundProp.sourceSpan));
              });
            }
            ...
            return directives.forEach(function(directive) {
              ...
              prop.isLiteral || boundDirectivePropsIndex.get(prop.name) || boundElementProps.push(_this._createElementPropertyAst(elementName, prop.name, prop.expression, prop.sourceSpan));
            }), boundElementProps;
            ...
            if (1 === parts.length) {
              ...
              if (_isAnimationLabel(partValue))
                boundPropertyName = partValue.substr(1), bindingType = exports.PropertyBindingType.Animation, securityContext = _angular_core.SecurityContext.NONE;
              else if (boundPropertyName = this._schemaRegistry.getMappedPropName(partValue), securityContext = this._schemaRegistry.securityContext(elementName, boundPropertyName), bindingType = exports.PropertyBindingType.Property, this._validatePropertyOrAttributeName(boundPropertyName, sourceSpan, !1), !this._schemaRegistry.hasProperty(elementName, boundPropertyName, this._schemas)) {
              ...
              boundPropertyName = parts[1], this._validatePropertyOrAttributeName(boundPropertyName, sourceSpan, !0);
            ...
            report.error && this._reportError(report.msg, sourceSpan, exports.ParseErrorLevel.FATAL);
            ...
            directives.forEach(function(directive) {
              ...
              !isPresent(event.target) && allDirectiveEvents.has(event.name) || _this._reportError("Event binding " + event.fullName + ' not emitted by any directive on an embedded template. Make sure that the event name is spelled correctly and all directives are listed in the "directives" section.', event.sourceSpan);
            });
        ...
        ElementContext = function() {
          ...
          return ElementContext.create = function(isTemplateElement, directives, providerContext) {
            ...
            }), ngContentIndices.sort(), isPresent(this._wildcardNgContentIndex) && ngContentIndices.push(this._wildcardNgContentIndex), ngContentIndices.length > 0 ? ngContentIndices[0] : null;
          }, ElementContext;
        }(),
        ...
        PipeCollector = function(_super) {
          function PipeCollector() {
            _super.apply(this, arguments), this.pipes = new Set;
          }
        ...
        CompilerConfig = function() {
          function CompilerConfig(_a) {
            var _b = void 0 === _a ? {} : _a,
                ...
                _e = _b.useJit,
                useJit = void 0 === _e || _e;
            this.renderTypes = renderTypes, this.defaultEncapsulation = defaultEncapsulation, this._genDebugInfo = genDebugInfo, this._logBindingUpdate = logBindingUpdate, this.useJit = useJit;
          }
        ...
        DefaultRenderTypes = function() {
          function DefaultRenderTypes() {
            this.renderText = null, this.renderElement = null, this.renderComment = null, this.renderNode = null, this.renderEvent = null;
          }
          return Object.defineProperty(DefaultRenderTypes.prototype, "renderer", {
            get: function() {
        ...
        __extends$12 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        AnimationStateAst = function(_super) {
          ...
          return __extends$12(AnimationStateAst, _super), AnimationStateAst;
        }(AnimationAst),
        AnimationEntryAst = function(_super) {
          ...
          return __extends$12(AnimationEntryAst, _super), AnimationEntryAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateDeclarationAst = function(_super) {
          ...
          return __extends$12(AnimationStateDeclarationAst, _super), AnimationStateDeclarationAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateTransitionAst = function(_super) {
          ...
          return __extends$12(AnimationStateTransitionAst, _super), AnimationStateTransitionAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStepAst = function(_super) {
          ...
          return __extends$12(AnimationStepAst, _super), AnimationStepAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStylesAst = function(_super) {
          ...
          return __extends$12(AnimationStylesAst, _super), AnimationStylesAst.prototype.visit = function(visitor, context) {
        ...
        AnimationKeyframeAst = function(_super) {
          ...
          return __extends$12(AnimationKeyframeAst, _super), AnimationKeyframeAst.prototype.visit = function(visitor, context) {
        ...
        AnimationWithStepsAst = function(_super) {
          ...
          return __extends$12(AnimationWithStepsAst, _super), AnimationWithStepsAst;
        }(AnimationAst),
        AnimationGroupAst = function(_super) {
          function AnimationGroupAst(steps) {
            _super.call(this, steps);
          }
          return __extends$12(AnimationGroupAst, _super), AnimationGroupAst.prototype.visit = function(visitor, context) {
        ...
        AnimationSequenceAst = function(_super) {
          ...
          return __extends$12(AnimationSequenceAst, _super), AnimationSequenceAst.prototype.visit = function(visitor, context) {
        ...
        AnimationEntryCompileResult = function() {
          function AnimationEntryCompileResult(name, statements, fnExp) {
            this.name = name, this.statements = statements, this.fnExp = fnExp;
          }
          return AnimationEntryCompileResult;
        }(),
        AnimationCompiler = function() {
          function AnimationCompiler() {}
          return AnimationCompiler.prototype.compile = function(factoryNamePrefix, parsedAnimations) {
            return parsedAnimations.map(function(entry) {
              var factoryName = factoryNamePrefix + "_" + entry.name,
                  visitor = new _AnimationBuilder(entry.name, factoryName);
        ...
        _ANIMATION_TIME_VAR = variable("totalTime"),
        ...
        _AnimationBuilder = function() {
          ...
          return _AnimationBuilder.prototype.visitAnimationStyles = function(ast, context) {
            ...
            }), importExpr(resolveIdentifier(Identifiers.AnimationStyles)).instantiate([importExpr(resolveIdentifier(Identifiers.collectAndResolveStyles)).callFn([_ANIMATION_COLLECTED_STYLES, literalArr(stylesArr)])]);
            ...
            var _this = this,
                ...
                keyframesExpr = importExpr(resolveIdentifier(Identifiers.balanceAnimationKeyframes)).callFn([_ANIMATION_COLLECTED_STYLES, _ANIMATION_END_STATE_STYLES_VAR, literalArr(keyframeExpressions)]);
            return this._callAnimateMethod(ast, startingStylesExpr, keyframesExpr, context);
          }, _AnimationBuilder.prototype._callAnimateMethod = function(ast, startingStylesExpr, keyframesExpr, context) {
            return context.totalTransitionTime += ast.duration + ast.delay, _ANIMATION_FACTORY_RENDERER_VAR.callMethod("animate", [_ANIMATION_FACTORY_ELEMENT_VAR, startingStylesExpr, keyframesExpr, literal(ast.duration), literal(ast.delay), literal(ast.easing)]);
            ...
            _isEndStateAnimateStep(lastStep) && (context.endStateAnimateStep = lastStep), context.totalTransitionTime = 0, context.isExpectingFirstStyleStep = !0;
            ...
            var animationPlayerExpr = ast.animation.visit(this, context),
                ...
                animationStmt = _ANIMATION_PLAYER_VAR.set(animationPlayerExpr).toStmt(),
                totalTimeStmt = _ANIMATION_TIME_VAR.set(literal(context.totalTransitionTime)).toStmt();
            ...
            var compiledStatesMapStmt = this._statesMapVar.set(literalMap(lookupMap)).toDeclStmt(),
                statements = [compiledStatesMapStmt, fnStatement];
            return new AnimationEntryCompileResult(this.animationName, statements, fnVariable);
          }, _AnimationBuilder;
        }(),
        _AnimationBuilderContext = function() {
          function _AnimationBuilderContext() {
            this.stateMap = new _AnimationBuilderStateMap, this.endStateAnimateStep = null, this.isExpectingFirstStyleStep = !1, this.totalTransitionTime = 0;
          }
        ...
        _AnimationBuilderStateMap = function() {
          ...
          return Object.defineProperty(_AnimationBuilderStateMap.prototype, "states", {
            ...
            existingEntry || (this._states[name] = value);
        ...
        __extends$13 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        AnimationParseError = function(_super) {
          ...
          return __extends$13(AnimationParseError, _super), AnimationParseError.prototype.toString = function() {
            return "" + this.msg;
          }, AnimationParseError;
        }(ParseError),
        ...
        AnimationParser = function() {
          ...
          return AnimationParser.prototype.parseComponent = function(component) {
            var _this = this,
                errors = [],
                componentName = component.type.name,
                animationTriggerNames = new Set,
                asts = component.template.animations.map(function(entry) {
                  var result = _this.parseEntry(entry),
                      ast = result.ast,
                      triggerName = ast.name;
                  if (animationTriggerNames.has(triggerName) ? result.errors.push(new AnimationParseError('The animation trigger "' + triggerName + '" has already been registered for the ' + componentName + " component")) : animationTriggerNames.add(triggerName), result.errors.length > 0) {
        ...
        DirectiveWrapperCompileResult = function() {
          function DirectiveWrapperCompileResult(statements, dirWrapperClassVar) {
            this.statements = statements, this.dirWrapperClassVar = dirWrapperClassVar;
          }
        ...
        CURR_VALUE_VAR = variable("currValue"),
        THROW_ON_CHANGE_VAR = variable("throwOnChange"),
        FORCE_UPDATE_VAR = variable("forceUpdate"),
        VIEW_VAR = variable("view"),
        RENDER_EL_VAR = variable("el"),
        RESET_CHANGES_STMT = THIS_EXPR.prop(CHANGES_FIELD_NAME).set(literalMap([])).toStmt(),
        DirectiveWrapperCompiler = function() {
          function DirectiveWrapperCompiler(compilerConfig) {
            this.compilerConfig = compilerConfig;
          }
          return DirectiveWrapperCompiler.dirWrapperClassName = function(id) {
            return "Wrapper_" + id.name;
          }, DirectiveWrapperCompiler.prototype.compile = function(dirMeta) {
            for (var dirDepParamNames = [],
                i = 0; i < dirMeta.type.diDeps.length; i++)
              dirDepParamNames.push("p" + i);
            var dirLifecycleHooks = dirMeta.type.lifecycleHooks,
                lifecycleHooks = {
                  genChanges: dirLifecycleHooks.indexOf(LifecycleHooks.OnChanges) !== -1 || this.compilerConfig.logBindingUpdate,
                ...
                fields = [new ClassField(CONTEXT_FIELD_NAME, importType(dirMeta.type)), new ClassField(CHANGED_FIELD_NAME, BOOL_TYPE)],
                ctorStmts = [THIS_EXPR.prop(CHANGED_FIELD_NAME).set(literal(!1)).toStmt()];
            lifecycleHooks.genChanges && (fields.push(new ClassField(CHANGES_FIELD_NAME, new MapType(DYNAMIC_TYPE))), ctorStmts.push(RESET_CHANGES_STMT));
            var methods = [];
            Object.keys(dirMeta.inputs).forEach(function(inputFieldName, idx) {
              var fieldName = "_" + inputFieldName;
              fields.push(new ClassField(fieldName, null, [StmtModifier.Private])), ctorStmts.push(THIS_EXPR.prop(fieldName).set(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))).toStmt()), methods.push(checkInputMethod(inputFieldName, THIS_EXPR.prop(fieldName), lifecycleHooks));
            }), methods.push(detectChangesInternalMethod(lifecycleHooks, this.compilerConfig.genDebugInfo)), ctorStmts.push(THIS_EXPR.prop(CONTEXT_FIELD_NAME).set(importExpr(dirMeta.type).instantiate(dirDepParamNames.map(function(paramName) {
              return variable(paramName);
            }))).toStmt());
            var ctor = new ClassMethod(null, dirDepParamNames.map(function(paramName) {
              return new FnParam(paramName, DYNAMIC_TYPE);
            }), ctorStmts),
                wrapperClassName = DirectiveWrapperCompiler.dirWrapperClassName(dirMeta.type),
                classStmt = new ClassStmt(wrapperClassName, null, fields, [], ctor, methods);
            return new DirectiveWrapperCompileResult([classStmt], wrapperClassName);
          }, DirectiveWrapperCompiler.decorators = [{type: _angular_core.Injectable}], DirectiveWrapperCompiler.ctorParameters = [{type: CompilerConfig}], DirectiveWrapperCompiler;
        }(),
        ...
        __extends$15 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        _ReplaceViewTransformer = function(_super) {
          function _ReplaceViewTransformer(_viewExpr, _view) {
            _super.call(this), this._viewExpr = _viewExpr, this._view = _view;
          }
          return __extends$15(_ReplaceViewTransformer, _super), _ReplaceViewTransformer.prototype._isThis = function(expr) {
            ...
            return this._isThis(ast.receiver) && (this._view.fields.some(function(field) {
              return field.name == ast.name;
            }) || this._view.getters.some(function(field) {
              return field.name == ast.name;
            })) ? this._viewExpr.cast(this._view.classType).prop(ast.name) : _super.prototype.visitReadPropExpr.call(this, ast, context);
        ...
        ViewConstructorVars = function() {
          ...
          return ViewConstructorVars.viewUtils = variable("viewUtils"), ViewConstructorVars.parentInjector = variable("parentInjector"), ViewConstructorVars.declarationEl = variable("declarationEl"), ViewConstructorVars;
        }(),
        ViewProperties = function() {
          function ViewProperties() {}
          return ViewProperties.renderer = THIS_EXPR.prop("renderer"), ViewProperties.projectableNodes = THIS_EXPR.prop("projectableNodes"), ViewProperties.viewUtils = THIS_EXPR.prop("viewUtils"), ViewProperties;
        }(),
        ...
        InjectMethodVars = function() {
          ...
          return InjectMethodVars.token = variable("token"), InjectMethodVars.requestNodeIndex = variable("requestNodeIndex"), InjectMethodVars.notFoundResult = variable("notFoundResult"), InjectMethodVars;
        }(),
        DetectChangesVars = function() {
          function DetectChangesVars() {}
          return DetectChangesVars.throwOnChange = variable("throwOnChange"), DetectChangesVars.changes = variable("changes"), DetectChangesVars.changed = variable("changed"), DetectChangesVars.valUnwrapper = variable("valUnwrapper"), DetectChangesVars;
        }(),
        ...
        DirectiveWrapperDependency = function() {
          function DirectiveWrapperDependency(dir, placeholder) {
            this.dir = dir, this.placeholder = placeholder;
          }
          return DirectiveWrapperDependency;
        }(),
        __extends$14 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        CompileElement = function(_super) {
          ...
          return __extends$14(CompileElement, _super), CompileElement.createNull = function() {
            ...
            this.view.createMethod.addStmt(statement), this.appElement = THIS_EXPR.prop(fieldName), this.instances.set(resolveIdentifierToken(Identifiers.AppElement).reference, this.appElement);
          }, CompileElement.prototype._createComponentFactoryResolver = function() {
            var _this = this,
                entryComponents = this.component.entryComponents.map(function(entryComponent) {
                  var id = new CompileIdentifierMetadata({name: entryComponent.name});
                  return _this._targetDependencies.push(new ComponentFactoryDependency(entryComponent, id)), id;
                });
            if (entryComponents && 0 !== entryComponents.length) {
              ...
              })), injectFromViewParentInjector(resolveIdentifierToken(Identifiers.ComponentFactoryResolver), !1)]),
            ...
            this._compViewExpr = compViewExpr, this.contentNodesByNgContentIndex = new Array(this.component.template.ngContentSelectors.length);
            ...
            var _this = this;
            this.hasViewContainer && this.instances.set(resolveIdentifierToken(Identifiers.ViewContainerRef).reference, this.appElement.prop("vcRef")), this._resolvedProviders = new Map, this._resolvedProvidersArray.forEach(function(provider) {
                  ...
                  providerValueExpressions = resolvedProvider.providers.map(function(provider) {
                    ...
                    if (isPresent(provider.useClass)) {
                      ...
                      if (isDirectiveWrapper) {
                        ...
                        return _this._targetDependencies.push(new DirectiveWrapperDependency(provider.useClass, directiveWrapperIdentifier)), importExpr(directiveWrapperIdentifier).instantiate(depsExpr, importType(directiveWrapperIdentifier));
                      }
              ...
              isDirectiveWrapper ? (_this.directiveWrapperInstance.set(resolvedProvider.token.reference, instance), _this.instances.set(resolvedProvider.token.reference, instance.prop("context"))) : _this.instances.set(resolvedProvider.token.reference, instance);
            });
            for (var i = 0; i < this._directives.length; i++) {
              var directive = this._directives[i],
                  directiveInstance = this.instances.get(identifierToken(directive.type).reference);
              directive.queries.forEach(function(queryMeta) {
                _this._addQuery(queryMeta, directiveInstance);
              });
            }
            var queriesWithReads = [];
            if (MapWrapper.values(this._resolvedProviders).forEach(function(resolvedProvider) {
              ...
              varValue = isPresent(token) ? _this.instances.get(token.reference) : _this.renderNode, _this.view.locals.set(varName, varValue);
              ...
              if (isPresent(queryWithRead.read.identifier))
                ...
                value = isPresent(token) ? _this.instances.get(token.reference) : _this.elementRef;
              }
            ...
            for (var queries,
              ...
              queries = currentEl._queries.get(token.reference), isPresent(queries) && ListWrapper.addAll(result, queries.filter(function(query) {
                return query.meta.descendants || distance <= 1;
              })), currentEl._directives.length > 0 && distance++, currentEl = currentEl.parent;
            return queries = this.view.componentView.viewQueries.get(token.reference), isPresent(queries) && ListWrapper.addAll(result, queries), result;
            ...
            var currElement = this,
                result = null;
            for (dep.isValue && (result = literal(dep.value)), result || dep.isSkipSelf || (result = this._getLocalDependency(requestingProviderType, dep)); !result && !currElement.parent.isNull(); )
            ...
            return result || (result = injectFromViewParentInjector(dep.token, dep.isOptional)), result || (result = NULL_EXPR), getPropertyInView(result, this.view, currElement.view);
          }, CompileElement;
        }(CompileNode),
        _QueryWithRead = function() {
          function _QueryWithRead(query, match) {
            this.query = query, this.read = query.meta.read || match;
          }
        ...
        CompilePipe = function() {
          function CompilePipe(view, meta) {
            ...
            var deps = this.meta.type.diDeps.map(function(diDep) {
              return diDep.token.reference === resolveIdentifierToken(Identifiers.ChangeDetectorRef).reference ? getPropertyInView(THIS_EXPR.prop("ref"), _this.view, _this.view.componentView) : injectFromViewParentInjector(diDep.token, !1);
            });
          ...
          return CompilePipe.call = function(view, name, args) {
            ...
            return meta.pure ? (pipe = compView.purePipes.get(name), pipe || (pipe = new CompilePipe(compView, meta), compView.purePipes.set(name, pipe), compView.pipes.push(pipe))) : (pipe = new CompilePipe(view, meta), view.pipes.push(pipe)), pipe._call(view, args);
            ...
            if (this.meta.pure) {
              ...
              return createPureProxy(pipeInstanceSeenFromPureProxy.prop("transform").callMethod(BuiltinMethod.Bind, [pipeInstanceSeenFromPureProxy]), args.length, purePipeProxyInstance, callingView), importExpr(resolveIdentifier(Identifiers.castByValue)).callFn([purePipeProxyInstance, pipeInstanceSeenFromPureProxy.prop("transform")]).callFn(args);
            }
        ...
        CompileView = function() {
          ...
          return CompileView.prototype.callPipe = function(name, input, args) {
            ...
            for (var currView = this,
                result = currView.locals.get(name); !result && isPresent(currView.declarationElement.view); )
        ...
        ExpressionWithWrappedValueInfo = function() {
          function ExpressionWithWrappedValueInfo(expression, needsValueUnwrapper, temporaryCount) {
            this.expression = expression, this.needsValueUnwrapper = needsValueUnwrapper, this.temporaryCount = temporaryCount;
          }
    ...
    var _ComponentIndex,
        _AstToIrVisitor = function() {
          function _AstToIrVisitor(_nameResolver, _implicitReceiver, _valueUnwrapper, bindingIndex) {
            this._nameResolver = _nameResolver, this._implicitReceiver = _implicitReceiver, this._valueUnwrapper = _valueUnwrapper, this.bindingIndex = bindingIndex, this._nodeMap = new Map, this._resultMap = new Map, this._currentTemporary = 0, this.needsValueUnwrapper = !1, this.temporaryCount = 0;
          }
          return _AstToIrVisitor.prototype.visitBinary = function(ast, mode) {
            ...
            return args.push(literal(ast.strings[ast.strings.length - 1])), importExpr(resolveIdentifier(Identifiers.interpolate)).callFn(args);
            ...
            return this.temporaryCount = Math.max(this._currentTemporary, this.temporaryCount), new ReadVarExpr(temporaryName(this.bindingIndex, tempNumber));
          }, _AstToIrVisitor.prototype.releaseTemporary = function(temporary) {
            if (this._currentTemporary--, temporary.name != temporaryName(this.bindingIndex, this._currentTemporary))
        ...
        CompileEventListener = function() {
          function CompileEventListener(compileElement, eventTarget, eventName, eventPhase, listenerIndex) {
            this.compileElement = compileElement, this.eventTarget = eventTarget, this.eventName = eventName, this.eventPhase = eventPhase, this._hasComponentHostListener = !1, this._actionResultExprs = [], this._method = new CompileMethod(compileElement.view), this._methodName = "_handle_" + sanitizeEventName(eventName) + "_" + compileElement.nodeIndex + "_" + listenerIndex, this._eventParam = new FnParam(EventHandlerVars.event.name, importType(this.compileElement.view.genConfig.renderTypes.renderEvent));
          }
          return CompileEventListener.getOrCreate = function(compileElement, eventTarget, eventName, eventPhase, targetEventListeners) {
            ...
            return listener || (listener = new CompileEventListener(compileElement, eventTarget, eventName, eventPhase, targetEventListeners.length), targetEventListeners.push(listener)), listener;
            ...
            var context = directiveInstance || this.compileElement.view.componentContext,
                actionStmts = convertCdStatementToIr(this.compileElement.view, context, hostEvent.handler, this.compileElement.nodeIndex),
            ...
            var callbackMethod = "start" == this.eventPhase ? "onStart" : "onDone";
        ...
        EvalResult = (not(DetectChangesVars.throwOnChange), function() {
        ...
        ViewBinderVisitor = function() {
          ...
          return ViewBinderVisitor.prototype.visitBoundText = function(ast, parent) {
            ...
            var compileElement = this.view.nodes[this._nodeIndex++],
                eventListeners = [];
            return collectEventListeners(ast.outputs, ast.directives, compileElement).forEach(function(entry) {
              eventListeners.push(entry);
            }), bindRenderInputs(ast.inputs, compileElement, eventListeners), bindRenderOutputs(eventListeners), ast.directives.forEach(function(directiveAst) {
              var directiveInstance = compileElement.instances.get(directiveAst.directive.type.reference),
                  directiveWrapperInstance = compileElement.directiveWrapperInstance.get(directiveAst.directive.type.reference);
              bindDirectiveInputs(directiveAst, directiveWrapperInstance, compileElement), bindDirectiveHostProps(directiveAst, directiveInstance, compileElement, eventListeners), bindDirectiveOutputs(directiveAst, directiveInstance, eventListeners);
        ...
        ViewBuilderVisitor = function() {
          function ViewBuilderVisitor(view, targetDependencies) {
            this.view = view, this.targetDependencies = targetDependencies, this.nestedViewCount = 0;
          }
          return ViewBuilderVisitor.prototype._isRootNode = function(parent) {
            ...
            this._isRootNode(parent) ? this.view.viewType !== ViewType.COMPONENT && this.view.rootNodesOrAppElements.push(vcAppEl || node.renderNode) : isPresent(parent.component) && isPresent(ngContentIndex) && parent.addContentNode(ngContentIndex, vcAppEl || node.renderNode);
            ...
            var renderNode = THIS_EXPR.prop(fieldName),
                ...
                compileElement = new CompileElement(parent, this.view, nodeIndex, renderNode, ast, null, directives, ast.providers, ast.hasViewContainer, (!0), ast.references, this.targetDependencies);
            this.view.nodes.push(compileElement), this.nestedViewCount++;
        ...
        ViewCompiler = function() {
          ...
          return ViewCompiler.prototype.compileComponent = function(component, template, styles, pipes, compiledAnimations) {
            var dependencies = [],
                view = new CompileView(component, this._genConfig, pipes, styles, compiledAnimations, 0, CompileElement.createNull(), []),
                statements = [];
          ...
          }, ViewCompiler.decorators = [{type: _angular_core.Injectable}], ViewCompiler.ctorParameters = [{type: CompilerConfig}], ViewCompiler;
        }(),
        ...
        NgModulesSummary = function() {
          function NgModulesSummary(ngModuleByDirective, ngModules) {
            this.ngModuleByDirective = ngModuleByDirective, this.ngModules = ngModules;
          }
        ...
        OfflineCompiler = function() {
          ...
          return OfflineCompiler.prototype.analyzeModules = function(ngModules) {
            ...
            var _this = this,
                fileSuffix = _splitTypescriptSuffix(moduleUrl)[1],
                statements = [],
                exportedVars = [],
                outputSourceModules = [];
            ...
            })), Promise.all(directives.map(function(dirType) {
            ...
            var ngModule = this._metadataResolver.getNgModuleMetadata(ngModuleType),
                providers = [];
            this._localeId && providers.push(new CompileProviderMetadata({
              token: resolveIdentifierToken(Identifiers.LOCALE_ID),
              useValue: this._localeId
            })), this._translationFormat && providers.push(new CompileProviderMetadata({
            ...
            var dirMeta = this._metadataResolver.getDirectiveMetadata(directiveType),
                dirCompileResult = this._dirWrapperCompiler.compile(dirMeta);
            return targetStatements.push.apply(targetStatements, dirCompileResult.statements), dirCompileResult.dirWrapperClassVar;
            ...
            var parsedAnimations = this._animationParser.parseComponent(compMeta),
                parsedTemplate = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, schemas, compMeta.type.name),
                stylesExpr = componentStyles ? variable(componentStyles.stylesVar) : literalArr([]),
                compiledAnimations = this._animationCompiler.compile(compMeta.type.name, parsedAnimations),
                viewResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, stylesExpr, pipes, compiledAnimations);
            return componentStyles && targetStatements.push.apply(targetStatements, _resolveStyleStatements(componentStyles, fileSuffix)), compiledAnimations.forEach(function(entry) {
              entry.statements.forEach(function(statement) {
                targetStatements.push(statement);
              });
            }), targetStatements.push.apply(targetStatements, _resolveViewStatements(viewResult)), viewResult.viewFactoryVar;
    ...
    var DirectiveNormalizer = function() {
      ...
      return DirectiveNormalizer.prototype.clearCache = function() {
        ...
        isBlank(encapsulation) && (encapsulation = this._config.defaultEncapsulation);
      ...
      }, DirectiveNormalizer.decorators = [{type: _angular_core.Injectable}], DirectiveNormalizer.ctorParameters = [{type: ResourceLoader}, {type: UrlResolver}, {type: HtmlParser}, {type: CompilerConfig}], DirectiveNormalizer;
    }(),
        ...
        DirectiveResolver = function() {
          ...
          return DirectiveResolver.prototype.resolve = function(type, throwIfNotFound) {
            ...
            return Object.keys(propertyMetadata).forEach(function(propName) {
              propertyMetadata[propName].forEach(function(a) {
                if (a instanceof _angular_core.Input)
                  a.bindingPropertyName ? inputs.push(propName + ": " + a.bindingPropertyName) : inputs.push(propName);
                else if (a instanceof _angular_core.Output) {
                  var output = a;
                  output.bindingPropertyName ? outputs.push(propName + ": " + output.bindingPropertyName) : outputs.push(propName);
                  ...
                  var hostListener = a,
                      args = hostListener.args || [];
                  host["(" + hostListener.eventName + ")"] = propName + "(" + args.join(",") + ")";
                } else
                  a instanceof _angular_core.Query && (queries[propName] = a);
              });
            ...
            var mergedHost = directive.host ? StringMapWrapper.merge(directive.host, host) : host,
                mergedQueries = directive.queries ? StringMapWrapper.merge(directive.queries, queries) : queries;
          ...
          }, DirectiveResolver.decorators = [{type: _angular_core.Injectable}], DirectiveResolver.ctorParameters = [{type: ReflectorReader}], DirectiveResolver;
        }(),
        NgModuleResolver = function() {
          ...
          return NgModuleResolver.prototype.resolve = function(type, throwIfNotFound) {
            ...
            return null;
          }, NgModuleResolver.decorators = [{type: _angular_core.Injectable}], NgModuleResolver.ctorParameters = [{type: ReflectorReader}], NgModuleResolver;
        }(),
        PipeResolver = function() {
          ...
          return PipeResolver.prototype.resolve = function(type, throwIfNotFound) {
            ...
            return null;
          }, PipeResolver.decorators = [{type: _angular_core.Injectable}], PipeResolver.ctorParameters = [{type: ReflectorReader}], PipeResolver;
        }(),
        __extends$16 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        CompileMetadataResolver = function() {
          function CompileMetadataResolver(_ngModuleResolver, _directiveResolver, _pipeResolver, _schemaRegistry, _reflector) {
            void 0 === _reflector && (_reflector = reflector), this._ngModuleResolver = _ngModuleResolver, this._directiveResolver = _directiveResolver, this._pipeResolver = _pipeResolver, this._schemaRegistry = _schemaRegistry, this._reflector = _reflector, this._directiveCache = new Map, this._pipeCache = new Map, this._ngModuleCache = new Map, this._ngModuleOfTypes = new Map, this._anonymousTypes = new Map, this._anonymousTypeIndex = 0;
          }
          return CompileMetadataResolver.prototype.sanitizeTokenName = function(token) {
            var identifier = stringify(token);
            if (identifier.indexOf("(") >= 0) {
              var found = this._anonymousTypes.get(token);
              found || (this._anonymousTypes.set(token, this._anonymousTypeIndex++), found = this._anonymousTypes.get(token)), identifier = "anonymous_token_" + found + "_";
            }
            ...
            if (!meta) {
              ...
              if (dirMeta instanceof _angular_core.Component) {
                ...
                }), changeDetectionStrategy = dirMeta.changeDetection, dirMeta.viewProviders && (viewProviders = this.getProvidersMetadata(dirMeta.viewProviders, entryComponentMetadata, 'viewProviders for "' + stringify(directiveType) + '"')), moduleUrl = componentModuleUrl(this._reflector, directiveType, dirMeta), dirMeta.entryComponents && (entryComponentMetadata = flattenArray(dirMeta.entryComponents).map(function(type) {
              ...
              var providers = [];
              isPresent(dirMeta.providers) && (providers = this.getProvidersMetadata(dirMeta.providers, entryComponentMetadata, 'providers for "' + stringify(directiveType) + '"'));
              var queries = [],
                  viewQueries = [];
            ...
            if (!compileMeta) {
              ...
              meta.imports && flattenArray(meta.imports).forEach(function(importedType) {
                ...
                if (isValidType(importedType))
                  ...
                  importedModuleType = moduleWithProviders.ngModule, moduleWithProviders.providers && providers_1.push.apply(providers_1, _this.getProvidersMetadata(moduleWithProviders.providers, entryComponents_1, "provider for the NgModule '" + stringify(importedModuleType) + "'"));
                }
                if (!importedModuleType)
                  throw new Error("Unexpected value '" + stringify(importedType) + "' imported by the module '" + stringify(moduleType) + "'");
                var importedMeta = _this.getNgModuleMetadata(importedModuleType, !1);
                if (null === importedMeta)
              ...
              if (meta.declarations && flattenArray(meta.declarations).forEach(function(declaredType) {
              ...
              }), meta.providers && providers_1.push.apply(providers_1, this.getProvidersMetadata(meta.providers, entryComponents_1, "provider for the NgModule '" + stringify(moduleType) + "'")), meta.entryComponents && entryComponents_1.push.apply(entryComponents_1, flattenArray(meta.entryComponents).map(function(type) {
                return _this.getTypeMetadata(type, staticTypeModuleUrl(type));
              })), meta.bootstrap) {
            ...
            return null !== this._directiveResolver.resolve(type, !1) ? "directive" : null !== this._pipeResolver.resolve(type, !1) ? "pipe" : null !== this._ngModuleResolver.resolve(type, !1) ? "module" : type.provide ? "provider" : "value";
            ...
            return void 0 === force && (force = !1), !(!force && transitiveModule.directivesSet.has(dirMeta.type.reference)) && (transitiveModule.directivesSet.add(dirMeta.type.reference), transitiveModule.directives.push(dirMeta), declaredDirectives.push(dirMeta), this._addTypeToModule(dirMeta.type.reference, moduleType), !0);
          }, CompileMetadataResolver.prototype._addPipeToModule = function(pipeMeta, moduleType, transitiveModule, declaredPipes, force) {
            return void 0 === force && (force = !1), !(!force && transitiveModule.pipesSet.has(pipeMeta.type.reference)) && (transitiveModule.pipesSet.add(pipeMeta.type.reference), transitiveModule.pipes.push(pipeMeta), declaredPipes.push(pipeMeta), this._addTypeToModule(pipeMeta.type.reference, moduleType), !0);
            ...
            var compileToken;
            return compileToken = new CompileTokenMetadata("string" == typeof token ? {value: token} : {identifier: new CompileIdentifierMetadata({
                reference: token,
            ...
            var _this = this,
                compileProviders = [];
            return providers.forEach(function(provider, providerIdx) {
              provider = _angular_core.resolveForwardRef(provider), provider && "object" == typeof provider && provider.hasOwnProperty("provide") && (provider = new ProviderMeta(provider.provide, provider));
              var compileProvider;
              if (Array.isArray(provider))
                ...
                tokenMeta.reference === resolveIdentifierToken(Identifiers.ANALYZE_FOR_ENTRY_COMPONENTS).reference ? targetEntryComponents.push.apply(targetEntryComponents, _this._getEntryComponentsFromProvider(provider)) : compileProvider = _this.getProviderMetadata(provider);
              } else {
                if (!isValidType(provider)) {
                  var providersInfo = providers.reduce(function(soFar, seenProvider, seenProviderIdx) {
                    return seenProviderIdx < providerIdx ? soFar.push("" + stringify(seenProvider)) : seenProviderIdx == providerIdx ? soFar.push("?" + stringify(seenProvider) + "?") : seenProviderIdx == providerIdx + 1 && soFar.push("..."), soFar;
                  }, []).join(", ");
          ...
          }, CompileMetadataResolver.decorators = [{type: _angular_core.Injectable}], CompileMetadataResolver.ctorParameters = [{type: NgModuleResolver}, {type: DirectiveResolver}, {type: PipeResolver}, {type: ElementSchemaRegistry}, {type: ReflectorReader}], CompileMetadataResolver;
        }(),
        _CompileValueConverter = function(_super) {
          function _CompileValueConverter() {
            _super.apply(this, arguments);
          }
          return __extends$16(_CompileValueConverter, _super), _CompileValueConverter.prototype.visitOther = function(value, targetIdentifiers) {
            var identifier;
            return identifier = new CompileIdentifierMetadata(isStaticSymbol(value) ? {
              name: value.name,
              moduleUrl: value.filePath,
              reference: value
            } : {reference: value}), targetIdentifiers.push(identifier), identifier;
        ...
        NgModuleCompiler = function() {
          ...
          }, NgModuleCompiler.decorators = [{type: _angular_core.Injectable}], NgModuleCompiler.ctorParameters = [], NgModuleCompiler;
        }(),
        _InjectorBuilder = function() {
          function _InjectorBuilder(_ngModuleMeta, _entryComponentFactories, _bootstrapComponentFactories, _sourceSpan) {
            this._ngModuleMeta = _ngModuleMeta, this._entryComponentFactories = _entryComponentFactories, this._bootstrapComponentFactories = _bootstrapComponentFactories, this._sourceSpan = _sourceSpan, this._tokens = [], this._instances = new Map, this._fields = [], this._createStmts = [], this._destroyStmts = [], this._getters = [];
          }
          return _InjectorBuilder.prototype.addProvider = function(resolvedProvider) {
            ...
            resolvedProvider.lifecycleHooks.indexOf(LifecycleHooks.OnDestroy) !== -1 && this._destroyStmts.push(instance.callMethod("ngOnDestroy", []).toStmt()), this._tokens.push(resolvedProvider.token), this._instances.set(resolvedProvider.token.reference, instance);
            ...
            var _this = this,
                ...
                ctor = new ClassMethod(null, [new FnParam(InjectorProps.parent.name, importType(resolveIdentifier(Identifiers.Injector)))], [SUPER_EXPR.callFn([variable(InjectorProps.parent.name), literalArr(this._entryComponentFactories.map(function(componentFactory) {
            ...
            var result = null;
            if (dep.isValue && (result = literal(dep.value)), dep.isSkipSelf || (!dep.token || dep.token.reference !== resolveIdentifierToken(Identifiers.Injector).reference && dep.token.reference !== resolveIdentifierToken(Identifiers.ComponentFactoryResolver).reference || (result = THIS_EXPR), result || (result = this._instances.get(dep.token.reference))), !result) {
        ...
        InjectorProps = function() {
          ...
          return InjectorProps.parent = THIS_EXPR.prop("parent"), InjectorProps;
        }(),
        InjectMethodVars$1 = function() {
          function InjectMethodVars() {}
          return InjectMethodVars.token = variable("token"), InjectMethodVars.notFoundResult = variable("notFoundResult"), InjectMethodVars;
        }(),
        ...
        AbstractEmitterVisitor = function() {
          ...
          return AbstractEmitterVisitor.prototype.visitExpressionStmt = function(stmt, ctx) {
            ...
            void 0 === absentValue && (absentValue = "null");
            var value = ast.value;
            return "string" == typeof value ? ctx.print(escapeIdentifier(value, this._escapeDollarInStrings)) : isBlank(value) ? ctx.print(absentValue) : ctx.print("" + value), null;
            ...
            return ctx.print("{", useNewLine), ctx.incIndent(), this.visitAllObjects(function(entry) {
              ctx.print(escapeIdentifier(entry[0], _this._escapeDollarInStrings, !1) + ": "), entry[1].visitExpression(_this, ctx);
        ...
        __extends$17 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        _TsEmitterVisitor = function(_super) {
          ...
          return __extends$17(_TsEmitterVisitor, _super), _TsEmitterVisitor.prototype.visitType = function(t, ctx, defaultType) {
            ...
            0 === ast.entries.length && ctx.print("(");
            var result = _super.prototype.visitLiteralArrayExpr.call(this, ast, ctx);
            return 0 === ast.entries.length && ctx.print(" as any[])"), result;
            ...
            value.reference && value.reference.members ? (ctx.print(value.reference.name), ctx.print("."), ctx.print(value.reference.members.join("."))) : ctx.print(value.name), isPresent(typeParams) && typeParams.length > 0 && (ctx.print("<"), this.visitAllObjects(function(type) {
        ...
        __extends$19 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        AbstractJsEmitterVisitor = function(_super) {
          ...
          return __extends$19(AbstractJsEmitterVisitor, _super), AbstractJsEmitterVisitor.prototype.visitDeclareClassStmt = function(stmt, ctx) {
            var _this = this;
        ...
        __extends$18 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        JitEmitterVisitor = function(_super) {
          function JitEmitterVisitor() {
            _super.apply(this, arguments), this._evalArgNames = [], this._evalArgValues = [];
          }
          return __extends$18(JitEmitterVisitor, _super), JitEmitterVisitor.prototype.getArgs = function() {
            for (var result = {},
        ...
        ShadowCss = function() {
          ...
          return ShadowCss.prototype.shimCssText = function(cssText, selector, hostSelector) {
            ...
            var attrName = "[" + scopeSelector + "]",
                _scopeSelectorPart = function(p) {
                  ...
                  if (p.indexOf(_polyfillHostNoCombinator) > -1)
                    ...
                    if (t.length > 0) {
                      var matches = t.match(/([^:]*)(:*)(.*)/);
                      null !== matches && (scopedP = matches[1] + attrName + matches[2] + matches[3]);
                    }
                  }
                  return scopedP;
                },
            ...
            selector = selector.replace(/\[[^\]]*\]/g, function(attrSelector) {
              ...
              return attrSelectors.push(attrSelector), attrSelectorIndex++, replaceBy;
            });
            for (var res,
                scopedSelector = "",
                startIndex = 0,
            ...
            return scopedSelector += _scopeSelectorPart(selector.substring(startIndex)), scopedSelector.replace(/__attr_sel_(\d+)__/g, function(ph, index) {
        ...
        StyleCompiler = function() {
          ...
          }, StyleCompiler.decorators = [{type: _angular_core.Injectable}], StyleCompiler.ctorParameters = [{type: UrlResolver}], StyleCompiler;
        }(),
        RuntimeCompiler = function() {
          ...
          return Object.defineProperty(RuntimeCompiler.prototype, "injector", {
            ...
            if (!ngModuleFactory) {
              ...
              compileResult.dependencies.forEach(function(dep) {
                dep.placeholder.reference = _this._assertComponentKnown(dep.comp.reference, !0).proxyComponentFactory, dep.placeholder.name = "compFactory_" + dep.comp.name;
            ...
            ngModule.transitiveModule.modules.forEach(function(localModuleMeta) {
              localModuleMeta.declaredDirectives.forEach(function(dirMeta) {
                moduleByDirective.set(dirMeta.type.reference, localModuleMeta), _this._compileDirectiveWrapper(dirMeta, localModuleMeta), dirMeta.isComponent && templates.add(_this._createCompiledTemplate(dirMeta, localModuleMeta));
              });
            }), ngModule.transitiveModule.modules.forEach(function(localModuleMeta) {
              localModuleMeta.declaredDirectives.forEach(function(dirMeta) {
            ...
            if (!compiledTemplate) {
              ...
              var hostMeta = createHostComponentMeta(compMeta);
              compiledTemplate = new CompiledTemplate((!0), compMeta.selector, compMeta.type, ngModule, [compMeta], this._templateNormalizer.normalizeDirective(hostMeta)), this._compiledHostTemplateCache.set(compType, compiledTemplate);
            }
            return compiledTemplate;
          }, RuntimeCompiler.prototype._createCompiledTemplate = function(compMeta, ngModule) {
            var compiledTemplate = this._compiledTemplateCache.get(compMeta.type.reference);
            return compiledTemplate || (assertComponent(compMeta), compiledTemplate = new CompiledTemplate((!1), compMeta.selector, compMeta.type, ngModule, ngModule.transitiveModule.directives, this._templateNormalizer.normalizeDirective(compMeta)), this._compiledTemplateCache.set(compMeta.type.reference, compiledTemplate)), compiledTemplate;
            ...
            var directiveWrapperClass,
                compileResult = this._directiveWrapperCompiler.compile(dirMeta),
                statements = compileResult.statements;
            directiveWrapperClass = this._compilerConfig.useJit ? jitStatements("/" + moduleMeta.type.name + "/" + dirMeta.type.name + "/wrapper.ngfactory.js", statements, compileResult.dirWrapperClassVar) : interpretStatements(statements, compileResult.dirWrapperClassVar), this._compiledDirectiveWrapperCache.set(dirMeta.type.reference, directiveWrapperClass);
            ...
            if (!template.isCompiled) {
              ...
              var viewCompMetas = template.viewComponentTypes.map(function(compType) {
                return _this._assertComponentLoaded(compType, !1).normalizedCompMeta;
                  ...
                  parsedAnimations = this._animationParser.parseComponent(compMeta),
                  ...
                  compiledAnimations = this._animationCompiler.compile(compMeta.type.name, parsedAnimations),
                  compileResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, variable(stylesCompileResult.componentStylesheet.stylesVar), template.viewPipes, compiledAnimations);
              compileResult.dependencies.forEach(function(dep) {
                var depTemplate;
                if (dep instanceof ViewFactoryDependency) {
                  var vfd = dep;
                  depTemplate = _this._assertComponentLoaded(vfd.comp.reference, !1), vfd.placeholder.reference = depTemplate.proxyViewFactory, vfd.placeholder.name = "viewFactory_" + vfd.comp.name;
                } else if (dep instanceof ComponentFactoryDependency) {
                  var cfd = dep;
                  depTemplate = _this._assertComponentLoaded(cfd.comp.reference, !0), cfd.placeholder.reference = depTemplate.proxyComponentFactory, cfd.placeholder.name = "compFactory_" + cfd.comp.name;
                } else if (dep instanceof DirectiveWrapperDependency) {
                  var dwd = dep;
              ...
              var factory;
              factory = this._compilerConfig.useJit ? jitStatements("/" + template.ngModule.type.name + "/" + template.compType.name + "/" + (template.isHost ? "host" : "component") + ".ngfactory.js", statements, compileResult.viewFactoryVar) : interpretStatements(statements, compileResult.viewFactoryVar), template.compiled(factory);
            }
            ...
            result.dependencies.forEach(function(dep, i) {
              ...
              dep.valuePlaceholder.reference = nestedStylesArr, dep.valuePlaceholder.name = "importedStyles" + i;
            });
          ...
          }, RuntimeCompiler.decorators = [{type: _angular_core.Injectable}], RuntimeCompiler.ctorParameters = [{type: _angular_core.Injector}, {type: CompileMetadataResolver}, {type: DirectiveNormalizer}, {type: TemplateParser}, {type: StyleCompiler}, {type: ViewCompiler}, {type: NgModuleCompiler}, {type: DirectiveWrapperCompiler}, {type: CompilerConfig}], RuntimeCompiler;
        }(),
        CompiledTemplate = function() {
          function CompiledTemplate(isHost, selector, compType, ngModule, viewDirectiveAndComponents, _normalizeResult) {
            ...
            }, this.proxyComponentFactory = isHost ? new _angular_core.ComponentFactory(selector, this.proxyViewFactory, compType.reference) : null, _normalizeResult.syncResult ? this._normalizedCompMeta = _normalizeResult.syncResult : this.loading = _normalizeResult.asyncResult.then(function(normalizedCompMeta) {
      ...
      d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
        ...
        DomElementSchemaRegistry = function(_super) {
          function DomElementSchemaRegistry() {
            var _this = this;
            _super.call(this), this._schema = {}, SCHEMA.forEach(function(encodedType) {
              var type = {},
          ...
          return __extends$20(DomElementSchemaRegistry, _super), DomElementSchemaRegistry.prototype.hasProperty = function(tagName, propName, schemaMetas) {
            ...
            return {error: !1};
          }, DomElementSchemaRegistry.decorators = [{type: _angular_core.Injectable}], DomElementSchemaRegistry.ctorParameters = [], DomElementSchemaRegistry;
